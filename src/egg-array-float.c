 ...
 **/
static gfloat egg_array_float_guassian_value(gfloat x, gfloat sigma)
{
    return (1.0 / (sqrtf(2.0 * 3.1415927) * sigma)) *
	(expf((-(powf(x, 2.0))) / (2.0 * powf(sigma, 2.0))));
}
 ...
 * @array: input array
 **/
gfloat egg_array_float_get(EggArrayFloat * array, guint i)
{
    if (i >= array->len)
 ...
 * @array: input array
 **/
void egg_array_float_set(EggArrayFloat * array, guint i, gfloat value)
{
 ...
 **/
EggArrayFloat *egg_array_float_compute_gaussian(guint length, gfloat sigma)
{
    EggArrayFloat *array;
    guint half_length;
    guint i;
	...
	egg_debug("half_length=%i, div=%f, sigma=%f", half_length,
		  division, sigma);
	g_array_index(array, gfloat, i) =
	    egg_array_float_guassian_value(division, sigma);
    }
	...
	egg_warning("got wrong sum (%f), perhaps sigma too high for size?",
		    value);
					...
					EggArrayFloat * kernel)
{
	...
	for (j = 0; j < length_kernel; j++) {
		...
		idx = length_data - 1;
	    value +=
		g_array_index(data, gfloat, idx) * g_array_index(kernel,
								 gfloat,
								 j);
	}
 ...
 * powfi:
 **/
static gfloat powfi(gfloat base, guint n)
{
    guint i;
    gfloat retval = 1;
    for (i = 1; i <= n; i++)
 ...
 **/
EggArrayFloat *egg_array_float_remove_outliers(EggArrayFloat * data,
					       guint length, gfloat sigma)
{
    guint i;
    guint j;
    guint half_length;
	...
	/* stddev is okay */
	if (value < sigma) {
	    g_array_index(result, gfloat, i) =
		g_array_index(data, gfloat, i);
	} else {
...
void egg_array_float_test(gpointer data)
{
	...
	egg_test_failed(test, "got gaussian array length (%i)",
			array->len);
    egg_array_float_print(array);
    ...
    size = 5;
    sigma = 1.1;
    egg_test_title(test, "get inprecise gaussian array (%i), sigma %f",
		   size, sigma);
    ...
    size = 9;
    sigma = 1.1;
    egg_test_title(test, "get gaussian-9 array (%i), sigma %f", size,
		   sigma);
    kernel = egg_array_float_compute_gaussian(size, sigma);
	...
	egg_test_failed(test, "got correct size convolve product (%f)",
			result->len);
	...
	egg_test_failed(test, "got incorrect size convolve product (%f)",
			result->len);
    egg_array_float_print(array);
    egg_array_float_print(result);
