 ...
 **/
gboolean
gpm_backlight_get_brightness(GpmBacklight * backlight, guint * brightness,
			     GError ** error)
{
    guint level;
    gboolean ret;
 ...
 **/
gboolean
gpm_backlight_set_brightness(GpmBacklight * backlight, guint percentage,
			     GError ** error)
{
    gboolean ret;
    gboolean hw_changed;
    ...
    ret =
	gpm_brightness_set(backlight->priv->brightness, percentage,
			   &hw_changed);
    if (!ret) {
	g_set_error_literal(error, gpm_backlight_error_quark(),
	...
	g_signal_emit(backlight, signals[BRIGHTNESS_CHANGED], 0,
		      percentage);
    }
    return ret;
}
...
static void
gpm_backlight_brightness_interact_panel(GpmBacklight * backlight, gint * x,
					...
					gint startx, gint starty)
{
    ...
    GError *error = NULL;
    position =
	g_dbus_proxy_call_sync(backlight->priv->proxy, "GetPanelPosition",
			       g_variant_new("(s)", "position"),
 ...
 **/
static void gpm_backlight_dialog_init(GpmBacklight * backlight)
{
	...
	msd_media_keys_window_set_action_custom(MSD_MEDIA_KEYS_WINDOW
 ...
 **/
static void gpm_backlight_dialog_show(GpmBacklight * backlight)
{
    int orig_w;
    int orig_h;
    int screen_w;
	...
	gtk_widget_get_style_context(GTK_WIDGET(backlight->priv->popup));
    gtk_style_context_save(context);
    GtkCssProvider *provider = gtk_css_provider_new();
    gtk_css_provider_load_from_data(provider,
				    ".volume-box {border-radius:6px;background:rgba(0,0,0,0.95);}",
				    -1, NULL);
     ...
     */
    gtk_window_get_default_size(GTK_WINDOW(backlight->priv->popup),
				&orig_w, &orig_h);
...
#if GTK_CHECK_VERSION (3, 22, 0)
    monitor =
	gdk_display_get_monitor_at_point(gdk_screen_get_display
    ...
    gdk_screen_get_monitor_geometry(pointer_screen, monitor, &geometry);
#endif
	...
	gpm_backlight_brightness_interact_panel(backlight, &x, &y,
						screen_w, screen_h,
...
static gboolean
gpm_backlight_brightness_half (GpmBacklight *backlight)
{
	gfloat brightness;
	gfloat scale;
	gboolean ret;
	...
	gpm_brightness_set (backlight->priv->brightness, value/2, &hw_changed);

	sleep(3);
	if(backlight->priv->tid!=0){
		backlight->priv->tid=0;
		return TRUE;
	}
	gpm_dpms_set_mode(backlight->priv->dpms, GPM_DPMS_MODE_OFF, &ret);
	return TRUE;
}
			    ...
			    GpmBacklight * backlight)
{
	...
	backlight->priv->master_percentage = backlight->priv->master_percentage<=10?10:backlight->priv->master_percentage;
	if (!on_battery)
	    ...
	    gpm_backlight_brightness_evaluate_and_set(backlight, FALSE,
	       ...
	       && g_strcmp0(key, GPM_SETTINGS_BRIGHTNESS_DIM_BATT) == 0) {
	...
	gpm_idle_set_timeout_dim(backlight->priv->idle,
				 backlight->priv->idle_dim_timeout);
    } else if(g_strcmp0 (key, GPM_SETTINGS_AC_POLICY) == 0 && !on_battery) {
        cur_md = g_settings_get_int (settings,key);
        if(cur_md == 1)//平衡模式
        {
            gboolean ret = gpm_power_policy(MODE_PERFORMANCE);
            g_settings_set_int (backlight->priv->settings, GPM_SETTINGS_MODE_POLICY,MODE_PERFORMANCE);
        }
        else if(cur_md == 2)//节能模式
        {
            gboolean ret = gpm_power_policy(MODE_SAVE);
            g_settings_set_int (backlight->priv->settings, GPM_SETTINGS_MODE_POLICY,MODE_SAVE);
        }
    }else if (g_strcmp0 (key, GPM_SETTINGS_BAT_POLICY) == 0 && on_battery) {
        cur_md = g_settings_get_int (settings,key);
        if(cur_md == 1)//平衡模式
        {
            gboolean ret = gpm_power_policy(MODE_PERFORMANCE);
...
gpm_backlight_client_changed_cb(UpClient * client, GParamSpec * pspec,
				GpmBacklight * backlight)
#else
gpm_backlight_client_changed_cb(UpClient * client,
				GpmBacklight * backlight)
#endif
{
    gboolean on_battery;
    gboolean lid_state;
    g_object_get(backlight->priv->client, "lid-is-closed", &lid_state, NULL);
    if(backlight->priv->lid_is_closed != lid_state){
	backlight->priv->lid_is_closed = lid_state;
        return ;
    }
    guint percentage;
    gpm_backlight_brightness_evaluate_and_set(backlight, FALSE, TRUE);
    ...
    if (!on_battery) {
        g_settings_set_double(backlight->priv->settings,
                  GPM_SETTINGS_BRIGHTNESS_AC,
                  percentage * 2.0 > 100 ? 100:percentage * 2.0);
    } else {
        g_settings_set_double(backlight->priv->settings,
                  GPM_SETTINGS_BRIGHTNESS_AC,
                  percentage * 1.0);
    ...
    g_object_get (backlight->priv->client, "on-battery", &on_battery, NULL);
    if (!on_battery) {
            cur_md = g_settings_get_int (backlight->priv->settings,GPM_SETTINGS_AC_POLICY);
    }else{
            cur_md = g_settings_get_int (backlight->priv->settings,GPM_SETTINGS_BAT_POLICY);
    }
    if(cur_md == 1)//平衡模式
    {
        gboolean ret = gpm_power_policy(MODE_PERFORMANCE);
 ...
 **/
static void
gpm_backlight_button_pressed_cb(GpmButton * button, const gchar * type,
				GpmBacklight * backlight)
{
    gboolean ret;
    GError *error = NULL;
	...
	if (ret) {
	    gpm_brightness_get(backlight->priv->brightness, &percentage);
	    gpm_backlight_dialog_init(backlight);
	    msd_media_keys_window_set_volume_level(MSD_MEDIA_KEYS_WINDOW
						   (backlight->priv->
						    popup), percentage);
	    gpm_backlight_dialog_show(backlight);
	    /* save the new percentage */
...
//                      }
	...
	/* go up down step */
	ret =
	    gpm_brightness_down(backlight->priv->brightness, &hw_changed);
	...
	if (ret) {
	    ...
	    gpm_backlight_dialog_show(backlight);
	    /* save the new percentage */
	    egg_debug("percentage1=%d",percentage);
	    backlight->priv->master_percentage = percentage;
	    g_settings_set_double(backlight->priv->settings,
				  GPM_SETTINGS_BRIGHTNESS_AC,
				  percentage * 1.0);
...
//                      }
	...
	ret =
	    gpm_dpms_set_mode(backlight->priv->dpms, GPM_DPMS_MODE_ON,
			      &error);
	if (!ret) {
...
//              }
	...
	g_object_get(backlight->priv->client,
		     "on-battery", &on_battery, NULL);
	if (!on_battery)
	    dpms_mode =
		g_settings_get_enum(backlight->priv->settings,
				    GPM_SETTINGS_DPMS_METHOD_AC);
	else
	    dpms_mode =
		g_settings_get_enum(backlight->priv->settings,
				    GPM_SETTINGS_DPMS_METHOD_BATT);

	/* check if method is valid */
	if (dpms_mode == GPM_DPMS_MODE_UNKNOWN
	    || dpms_mode == GPM_DPMS_MODE_ON) {
	    egg_warning("BACKLIGHT method %i unknown. Using OFF.",
			dpms_mode);
	    dpms_mode = GPM_DPMS_MODE_OFF;
	}
	pthread_create(&backlight->priv->t,NULL,(void *)gpm_backlight_brightness_half,backlight);
 ...
 **/
static void
brightness_changed_cb(GpmBrightness * brightness, guint percentage,
		      GpmBacklight * backlight)
{
    /* save the new percentage */
    backlight->priv->master_percentage = percentage;
 ...
 **/
static void
control_resume_cb(GpmControl * control, GpmControlAction action,
		  GpmBacklight * backlight)
{
    gboolean ret;
    GError *error = NULL;
 ...
 **/
static void gpm_backlight_finalize(GObject * object)
{
    GpmBacklight *backlight;
 ...
 **/
static void gpm_backlight_class_init(GpmBacklightClass * klass)
{
	...
	g_signal_new("brightness-changed",
		     ...
		     G_SIGNAL_RUN_LAST,
		     G_STRUCT_OFFSET(GpmBacklightClass,
				     brightness_changed), NULL, NULL,
 ...
 **/
static void gpm_backlight_init(GpmBacklight * backlight)
{
    ...
    /* gets caps */
    backlight->priv->can_dim =
	gpm_brightness_has_hw(backlight->priv->brightness);

    /* watch for dim value changes */
    backlight->priv->settings = g_settings_new(GPM_SETTINGS_SCHEMA);
    g_signal_connect(backlight->priv->settings, "changed",
		     G_CALLBACK(gpm_settings_key_changed_cb), backlight);
    ...
    backlight->priv->proxy =
	g_dbus_proxy_new_for_bus_sync(G_BUS_TYPE_SESSION,
				      G_DBUS_PROXY_FLAGS_DO_NOT_LOAD_PROPERTIES,
				      NULL, "com.ukui.panel.desktop", "/",
				      "com.ukui.panel.desktop", NULL,
				      &error);
    if (backlight->priv->proxy == NULL) {
	if (error != NULL) {
	    egg_debug("error connecting to dbus - %s", error->message);
    ...
    g_object_get(backlight->priv->client, "lid-is-closed", &backlight->priv->lid_is_closed, NULL);
}
