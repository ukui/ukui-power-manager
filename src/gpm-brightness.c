 ...
 **/
static gint gpm_brightness_helper_get_value(const gchar * argument)
{
    gboolean ret;
    GError *error = NULL;
    ...
    /* get the data */
    command =
	g_strdup_printf(SBINDIR "/ukui-power-backlight-helper --%s",
			argument);
    ret =
	g_spawn_command_line_sync(command, &stdout_data, NULL,
				  &exit_status, &error);
    if (!ret) {
...
gpm_brightness_helper_set_value(const gchar * argument, gint value)
{
    ...
    /* get the data */
    value = value<=12000?12000:value;
    command =
	g_strdup_printf("pkexec " SBINDIR
			"/ukui-power-backlight-helper --%s %i", argument,
			value);
    ret =
	g_spawn_command_line_sync(command, NULL, NULL, &exit_status,
				  &error);
    if (!ret) {
	egg_error("failed to get value: %s", error->message);
	g_error_free(error);
 ...
 **/
static guint gpm_brightness_get_step(guint levels)
{
 ...
 **/
static gboolean
gpm_brightness_output_get_internal(GpmBrightness * brightness,
				   RROutput output, guint * cur)
{
	...
	(brightness->priv->dpy, output, brightness->priv->backlight, 0, 4,
	 False, False, None, &actual_type, &actual_format, &nitems,
 ...
 **/
static gboolean
gpm_brightness_output_set_internal(GpmBrightness * brightness,
				   RROutput output, guint value)
{
    gboolean ret = TRUE;
    ...
    gdk_error_trap_push();
    XRRChangeOutputProperty(brightness->priv->dpy, output,
			    brightness->priv->backlight, XA_INTEGER, 32,
			    PropModeReplace, (unsigned char *) &value, 1);
    XFlush(brightness->priv->dpy);
    gdk_flush();
    if (gdk_error_trap_pop()) {
	egg_warning("failed to XRRChangeOutputProperty for brightness %i",
		    value);
	ret = FALSE;
    }
 ...
 **/
static gboolean gpm_brightness_setup_display(GpmBrightness * brightness)
{
    gint major, minor;
	...
	XInternAtom(brightness->priv->dpy, "Backlight", True);
    if (brightness->priv->backlight == None) {
 ...
 **/
static gboolean
gpm_brightness_output_get_limits(GpmBrightness * brightness,
				 RROutput output, guint * min, guint * max)
{
    ...
    info =
	XRRQueryOutputProperty(brightness->priv->dpy, output,
			       brightness->priv->backlight);
    if (info == NULL) {
 ...
 **/
static gboolean
gpm_brightness_output_get_percentage(GpmBrightness * brightness,
				     RROutput output)
{
    guint cur;
    gboolean ret;
...
gpm_brightness_output_set(GpmBrightness * brightness, RROutput output)
{
    ...
    shared_value_abs =
	egg_discrete_from_percent(brightness->priv->shared_value,
				  (max - min) + 1);
    egg_debug("percent=%i, absolute=%i", brightness->priv->shared_value,
	      shared_value_abs);
	...
	for (i = cur; i <= shared_value_abs; i += step) {
	    ret =
		gpm_brightness_output_set_internal(brightness, output, i);
	    if (!ret)
		break;
	    if ((gint) cur != shared_value_abs)
 ...
 **/
static gboolean
gpm_brightness_foreach_resource(GpmBrightness * brightness, GpmXRandROp op,
...
gpm_brightness_foreach_screen(GpmBrightness * brightness, GpmXRandROp op)
{
    ...
    for (i = 0; i < length; i++) {
	resource =
	    (XRRScreenResources *) g_ptr_array_index(brightness->priv->
						     resources, i);
 ...
 **/
static gboolean gpm_brightness_trust_cache(GpmBrightness * brightness)
{
    g_return_val_if_fail(GPM_IS_BRIGHTNESS(brightness), FALSE);
    /* only return the cached value if the cache is trusted and we have change events */
    if (brightness->priv->cache_trusted
	&& brightness->priv->has_changed_events) {
	egg_debug("using cache for value %u (okay)",
		  brightness->priv->cache_percentage);
	return TRUE;
    }
	...
	egg_warning("using cache for value %u (probably okay)",
		    brightness->priv->cache_percentage);
	return TRUE;
    }
    return FALSE;
		   ...
		   gboolean * hw_changed)
{
	...
	if (brightness->priv->extension_levels < 0)
		...
		gpm_brightness_helper_get_value("get-max-brightness");
	brightness->priv->extension_current =
	    egg_discrete_from_percent(percentage,
				      brightness->priv->extension_levels +
				      1);
	ret =
	    gpm_brightness_helper_set_value("set-brightness",
					    brightness->priv->
					    extension_current);
    }
 ...
 **/
gboolean gpm_brightness_get(GpmBrightness * brightness, guint * percentage)
{
    gboolean ret = FALSE;
    gboolean trust_cache;
	...
	*percentage = brightness->priv->cache_percentage;
	...
	if (brightness->priv->extension_levels < 0)
		...
		gpm_brightness_helper_get_value("get-max-brightness");
	brightness->priv->extension_current =
	    gpm_brightness_helper_get_value("get-brightness");
	percentage_local =
	    egg_discrete_to_percent(brightness->priv->extension_current,
				    brightness->priv->extension_levels +
				    1);
	ret = TRUE;
    }
	...
	egg_warning("percentage value of %i will be truncated",
		    percentage_local);
...
gpm_brightness_up(GpmBrightness * brightness, gboolean * hw_changed)
{
	...
	*hw_changed = brightness->priv->hw_changed;
	...
	if (brightness->priv->extension_current <
	    brightness->priv->extension_levels) {
	    step =
		gpm_brightness_get_step(brightness->priv->
					extension_levels);
...
gpm_brightness_down(GpmBrightness * brightness, gboolean * hw_changed)
{
	...
	*hw_changed = brightness->priv->hw_changed;
	...
	/*最低亮度设定为步长 */
	gint min_brightness =
	    gpm_brightness_get_step(brightness->priv->extension_levels);
...
gpm_brightness_filter_xevents(GdkXEvent * xevent, GdkEvent * event,
			      gpointer data)
{
 ...
 **/
static void
gpm_brightness_monitors_changed(GdkScreen * screen,
...
static void gpm_brightness_update_cache(GpmBrightness * brightness)
{
    ...
    if (g_object_get_data(G_OBJECT(gscreen), "gpk-set-monitors-changed") ==
	NULL) {
	...
	g_signal_connect(G_OBJECT(gscreen), "monitors_changed",
			 G_CALLBACK(gpm_brightness_monitors_changed),
			 brightness);
    }
...
static void gpm_brightness_class_init(GpmBrightnessClass * klass)
{
	...
	g_signal_new("brightness-changed",
		     ...
		     G_STRUCT_OFFSET(GpmBrightnessClass,
...
static void gpm_brightness_init(GpmBrightness * brightness)
{
    ...
    brightness->priv->resources =
	g_ptr_array_new_with_free_func((GDestroyNotify)
				       XRRFreeScreenResources);

    /* can we do this */
    brightness->priv->has_extension =
	gpm_brightness_setup_display(brightness);
	...
	(GDK_DISPLAY_XDISPLAY(gdk_display_get_default()), &event_base,
	 &ignore)) {
	egg_warning("can't get event_base for XRR");
    }
    gdk_x11_register_standard_event_type(display, event_base,
					 RRNotify + 1);
    ...
    /* don't abort on error */
    gdk_error_trap_push();
    XRRSelectInput(GDK_DISPLAY_XDISPLAY(gdk_display_get_default()), GDK_WINDOW_XID(brightness->priv->root_window), RRScreenChangeNotifyMask | RROutputPropertyNotifyMask);	/* <--- the only one we need, but see rh:345551 */
    gdk_flush();
    if (gdk_error_trap_pop())
...
GpmBrightness *gpm_brightness_new(void)
{
	...
	g_object_add_weak_pointer(gpm_brightness_object,
				  &gpm_brightness_object);
    }
