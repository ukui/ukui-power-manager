...
gpm_button_emit_type(GpmButton * button, const gchar * type)
{
	...
	g_timer_elapsed(button->priv->timer,
			NULL) < GPM_BUTTON_DUPLICATE_TIMEOUT) {
...
gpm_button_filter_x_events(GdkXEvent * xevent, GdkEvent * event,
			   gpointer data)
{
    ...
    key =
	g_hash_table_lookup(button->priv->keysym_to_name_hash,
			    (gpointer) keycode_str);
    g_free(keycode_str);
...
gpm_button_grab_keystring(GpmButton * button, guint64 keycode)
{
    ...
    egg_debug("Grabbed modmask=%x, keycode=%li", modmask,
	      (long int) keycode);
    return TRUE;
}
 ...
 **/
static gboolean
gpm_button_xevent_key(GpmButton * button, guint keysym,
		      const gchar * key_name)
{
    ...
    /* add to hash table */
    g_hash_table_insert(button->priv->keysym_to_name_hash,
			(gpointer) keycode_str,
			(gpointer) g_strdup(key_name));
...
gpm_button_client_changed_cb(UpClient * client, GParamSpec * pspec,
			     GpmButton * button)
#else
...
static void
gpm_button_brightness_cb(GDBusProxy * proxy,
			 gchar * sender_name,
			 gchar * signal_name,
			 GVariant * parameters, gpointer user_data)
{
    ...
    guint value;
    GpmButton *button = GPM_BUTTON(user_data);
    if (g_strcmp0(signal_name, "brightnessUp") == 0) {
	gpm_button_emit_type(button, GPM_BUTTON_BRIGHT_UP);
    } else if (g_strcmp0(signal_name, "brightnessDown") == 0) {

	gpm_button_emit_type(button, GPM_BUTTON_BRIGHT_DOWN);
    }
...
static void gpm_button_init(GpmButton * button)
{
    ...
    button->priv->window =
	gdk_screen_get_root_window(button->priv->screen);

    button->priv->keysym_to_name_hash =
	g_hash_table_new_full(g_str_hash, g_str_equal, g_free, g_free);
    button->priv->last_button = NULL;
    button->priv->timer = g_timer_new();

    button->priv->client = up_client_new();
    button->priv->lid_is_closed =
	up_client_get_lid_is_closed(button->priv->client);
#if UP_CHECK_VERSION(0, 99, 0)
    ...
    GError *error = NULL;
    button->priv->brightness_proxy =
	g_dbus_proxy_new_for_bus_sync(G_BUS_TYPE_SESSION,
				      G_DBUS_PROXY_FLAGS_DO_NOT_LOAD_PROPERTIES,
				      NULL, "org.ukui.SettingsDaemon",
				      "/org/ukui/SettingsDaemon/wayland",
				      "org.ukui.SettingsDaemon.wayland",
				      NULL, &error);
    if (button->priv->brightness_proxy == NULL) {
	g_error_free(error);
    } else {
	g_signal_connect(button->priv->brightness_proxy,
			 "g-signal",
			 G_CALLBACK(gpm_button_brightness_cb), button);
    }
