...
static UpDevice *gpm_engine_get_composite_device(GpmEngine * engine,
						 UpDevice *
						 original_device);
...
static void gpm_engine_device_changed_cb(UpDevice * device,
					 GParamSpec * pspec,
					 GpmEngine * engine);
...
gpm_engine_get_warning_time(GpmEngine * engine, UpDevice * device)
{
    ...
    /* get device properties */
    g_object_get(device,
		 "kind", &kind, "time-to-empty", &time_to_empty, NULL);

    /* this is probably an error condition */
    if (time_to_empty == 0) {
	//egg_debug("time zero, falling back to percentage for %s",
...
gpm_engine_get_warning(GpmEngine * engine, UpDevice * device)
{
    ...
    /* get device properties */
    g_object_get(device, "kind", &kind, "state", &state, NULL);
    ...
    if (kind == UP_DEVICE_KIND_MOUSE || kind == UP_DEVICE_KIND_KEYBOARD) {
...
gchar *gpm_engine_get_summary(GpmEngine * engine)
{
	...
	g_object_get(device,
		     "is-present", &is_present, "state", &state, NULL);
	if (!is_present)
	    continue;
...
#endif
{
	...
	g_object_get(device,
		     "kind", &kind,
		     "state", &state, "is-present", &is_present, NULL);
	...
	warning_temp =
	    GPOINTER_TO_INT(g_object_get_data
			    (G_OBJECT(device), "engine-warning-old"));
...
gchar *gpm_engine_get_icon(GpmEngine * engine)
{
    ...
    icon =
	gpm_engine_get_icon_priv(engine, UP_DEVICE_KIND_BATTERY,
				 GPM_ENGINE_WARNING_CRITICAL, FALSE);
    if (icon != NULL)
	return icon;
    icon =
	gpm_engine_get_icon_priv(engine, UP_DEVICE_KIND_UPS,
				 GPM_ENGINE_WARNING_CRITICAL, FALSE);
    if (icon != NULL)
	return icon;
    icon =
	gpm_engine_get_icon_priv(engine, UP_DEVICE_KIND_MOUSE,
				 GPM_ENGINE_WARNING_CRITICAL, FALSE);
    if (icon != NULL)
	return icon;
    icon =
	gpm_engine_get_icon_priv(engine, UP_DEVICE_KIND_KEYBOARD,
				 GPM_ENGINE_WARNING_CRITICAL, FALSE);
    if (icon != NULL)
	return icon;
    ...
    icon =
	gpm_engine_get_icon_priv(engine, UP_DEVICE_KIND_BATTERY,
				 GPM_ENGINE_WARNING_NONE, FALSE);
    if (icon != NULL) {
	//kobe
	if (engine->priv->icon_policy == GPM_ICON_POLICY_PRESENT) {	//如果用户设置为仅使用电池时显示其图标，则需要进行以下处理
	    UpDeviceState state;
	    g_object_get(engine->priv->battery_composite, "state", &state,
			 NULL);
	    if (state != UP_DEVICE_STATE_DISCHARGING) {
		return NULL;
	    }
 ...
 */
static gboolean gpm_engine_recalculate_state_icon(GpmEngine * engine)
{
    gchar *icon;
 ...
 */
static gboolean gpm_engine_recalculate_state_summary(GpmEngine * engine)
{
    gchar *summary;
 ...
 */
static void gpm_engine_recalculate_state(GpmEngine * engine)
{
				   ...
				   GpmEngine * engine)
{
	...
	engine->priv->use_time_primary =
	    g_settings_get_boolean(settings, key);

    }else if(g_strcmp0(key, GPM_SETTINGS_PERCENTAGE_ACTION) == 0){
    
    	engine->priv->action_percentage =
            g_settings_get_int(settings, key);
	//egg_debug("percentage-action changed to %d",engine->priv->action_percentage);
    }
...
gpm_engine_device_check_capacity(GpmEngine * engine, UpDevice * device)
{
    ...
    /* get device properties */
    g_object_get(device, "kind", &kind, "capacity", &capacity, NULL);
    ...
    ret =
	g_settings_get_boolean(engine->priv->settings,
			       GPM_SETTINGS_NOTIFY_LOW_CAPACITY);
    if (ret) {
						 ...
						 original_device)
{
    ...
    g_object_get(original_device, "kind", &original_kind, NULL);
						    ...
						    original_device)
{
	...
	if (state == UP_DEVICE_STATE_DISCHARGING)
	    ...
	    time_to_full =
		3600 * ((energy_full_total - energy_total) /
			energy_rate_total);
    }
    ...
    g_object_set(device,
		 ...
		 "percentage", percentage, "state", state, NULL);
    if (debug) {
	text = up_device_to_text(device);
	//egg_debug("composite:\n%s", text);
	g_free(text);
 ...
 **/
static void gpm_engine_device_add(GpmEngine * engine, UpDevice * device)
{
#if UP_CHECK_VERSION (0, 99, 0)
    UpDeviceLevel warning;
#else
    ...
    g_object_set_data(G_OBJECT(device), "engine-warning-old",
    ...
    g_object_set_data(G_OBJECT(device), "engine-state-old",
	...
	g_object_set_data(G_OBJECT(composite), "engine-warning-old",
			  GUINT_TO_POINTER(warning));
 ...
 **/
static gboolean gpm_engine_coldplug_idle_cb(GpmEngine * engine)
{
    guint i;
    ...
    ret =
	up_client_enumerate_devices_sync(engine->priv->client, NULL,
					 &error);
    if (!ret) {
 ...
 **/
static void
gpm_engine_device_added_cb(UpClient * client, UpDevice * device,
			   GpmEngine * engine)
{
#if UP_CHECK_VERSION (0, 99, 0)
    gpm_engine_device_add(engine, device);
...
#else
gpm_engine_device_removed_cb(UpClient * client, UpDevice * device,
			     GpmEngine * engine)
{
    gboolean ret;
...
//kobe
static void
gpm_engine_device_monitor_cb(UpClient * client, UpDevice * device,
			     GpmEngine * engine)
{
    gpm_engine_recalculate_state(engine);
}
...
static void
#if UP_CHECK_VERSION(0, 99, 0)
gpm_engine_device_changed_cb(UpDevice * device, GParamSpec * pspec,
			     GpmEngine * engine)
#else
gpm_engine_device_changed_cb(UpClient * client, UpDevice * device,
			     GpmEngine * engine)
#endif
{
    UpDeviceKind kind;
    ...
    state_old =
	GPOINTER_TO_INT(g_object_get_data
			(G_OBJECT(device), "engine-state-old"));
    if (state_old != state) {
	if (state == UP_DEVICE_STATE_DISCHARGING) {
...
UpDevice *gpm_engine_get_primary_device(GpmEngine * engine)
{
	...
	device =
	    g_object_ref(gpm_engine_get_composite_device
			 (engine, device_tmp));
	break;
    }
    return device;
...
static void gpm_engine_init(GpmEngine * engine)
{
...
#if UP_CHECK_VERSION(0, 99, 0)
    engine->priv->battery_composite =
	up_client_get_display_device(engine->priv->client);
    ...
    g_object_set(engine->priv->battery_composite,
		 ...
		 "power-supply", TRUE, "is-present", TRUE, NULL);
#endif
    ...
    /* do we want to display the icon in the tray */
    engine->priv->icon_policy = GPM_ICON_POLICY_NEVER;	//g_settings_get_enum (engine->priv->settings, GPM_SETTINGS_ICON_POLICY);

    /* get percentage policy */
    engine->priv->low_percentage =
	g_settings_get_int(engine->priv->settings,
			   GPM_SETTINGS_PERCENTAGE_LOW);
    engine->priv->critical_percentage =
	g_settings_get_int(engine->priv->settings,
			   GPM_SETTINGS_PERCENTAGE_CRITICAL);
    engine->priv->action_percentage =
	g_settings_get_int(engine->priv->settings,
			   GPM_SETTINGS_PERCENTAGE_ACTION);
    if (engine->priv->action_percentage >=
	engine->priv->critical_percentage) {
	engine->priv->action_percentage =
	    engine->priv->critical_percentage - 1;

    }

    /* get time policy */
    engine->priv->low_time =
	g_settings_get_int(engine->priv->settings, GPM_SETTINGS_TIME_LOW);
    engine->priv->critical_time =
	g_settings_get_int(engine->priv->settings,
			   GPM_SETTINGS_TIME_CRITICAL);
    engine->priv->action_time =
	g_settings_get_int(engine->priv->settings,
			   GPM_SETTINGS_TIME_ACTION);

    /* we can disable this if the time remaining is inaccurate or just plain wrong */
    engine->priv->use_time_primary =
	g_settings_get_boolean(engine->priv->settings,
			       GPM_SETTINGS_USE_TIME_POLICY);
    if (engine->priv->use_time_primary)
...
static void gpm_engine_class_init(GpmEngineClass * klass)
{
	...
	g_signal_new("charge-critical-notify",
		     G_TYPE_FROM_CLASS(object_class),
		     G_SIGNAL_RUN_LAST,
		     G_STRUCT_OFFSET(GpmEngineClass,
				     charge_critical_notify), NULL, NULL,
		     g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1,
		     G_TYPE_POINTER);
    signals[CLOSE_NOTIFY] =
	g_signal_new("close-notify", G_TYPE_FROM_CLASS(object_class),
