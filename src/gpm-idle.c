...
gpm_idle_control (GpmIdle *idle)
{
    ...
    if (idle->priv->timeout_blank_id == 0 &&
        ...
        if (timeout_blank <= 0) timeout_blank = 1;
    ...
    /* only do the sleep timeout when the session is idle and we aren't inhibited from sleeping */
        if (idle->priv->timeout_sleep_id == 0 &&
            ...
            if(timeout_sleep <= 0)  timeout_sleep = 1;
            idle->priv->timeout_sleep_id = g_timeout_add_seconds (timeout_sleep,
...
gpm_idle_init (GpmIdle *idle)
{
    ...
    g_signal_connect (idle->priv->session, "control-changed", G_CALLBACK (gpm_idle_session_control_changed_cb), idle);
