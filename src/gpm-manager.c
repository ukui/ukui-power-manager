 ...
 **/
static gboolean gpm_manager_play_loop_timeout_cb(GpmManager * manager)
{
    ca_context *context;
 ...
 **/
static gboolean gpm_manager_play_loop_stop(GpmManager * manager)
{
 ...
 **/
static gboolean
gpm_manager_play_loop_start(GpmManager * manager, GpmManagerSound action,
			    gboolean force, guint timeout)
{
    const gchar *id = NULL;
    ...
    if (manager->priv->critical_alert_timeout_id != 0) {
	egg_warning
	    ("was instructed to play a sound loop with one already playing");
 ...
 **/
static gboolean
gpm_manager_play(GpmManager * manager, GpmManagerSound action,
		 gboolean force)
{
    const gchar *id = NULL;
    const gchar *desc = NULL;
	...
	id = "power-plug";
	/* TRANSLATORS: this is the sound description */
	manager->priv->pre_power_supply_ac = 1;
 ...
 **/
static gboolean
gpm_manager_is_inhibit_valid(GpmManager * manager, gboolean user_action,
			     const char *action)
{
    return TRUE;
}
 ...
 **/
static void gpm_manager_sync_policy_sleep(GpmManager * manager)
{
    guint sleep_display;
	...
	sleep_computer =
	    g_settings_get_int(manager->priv->settings,
    ...
    if(gpm_session_get_idle(manager->priv->session)){
	if(sleep_display != 0)sleep_display += 60;
        if(sleep_computer != 0)sleep_computer += 60;
    }
...
gpm_manager_unblank_screen(GpmManager * manager, GError ** noerror)
{
    ...
    do_lock =
	gpm_control_get_lock_policy(manager->priv->control,
				    GPM_SETTINGS_LOCK_ON_BLANK_SCREEN);
    if (do_lock)
 ...
 **/
static gboolean
gpm_manager_notify_close(GpmManager * manager,
 ...
 **/
static void
gpm_manager_notification_closed_cb(NotifyNotification * notification,
				   NotifyNotification **
				   notification_class)
{
 ...
 **/
static gboolean
gpm_manager_notify(GpmManager * manager,
		   NotifyNotification ** notification_class,
		   const gchar * title, const gchar * message,
		   guint timeout, const gchar * icon,
		   NotifyUrgency urgency)
{
	...
	notification =
	    notify_notification_new(title, message,
				    gtk_status_icon_get_icon_name(manager->
								  priv->
								  status_icon));
    else
    ...
    g_signal_connect(notification, "closed",
		     G_CALLBACK(gpm_manager_notification_closed_cb),
		     notification_class);
 ...
 **/
static void
gpm_manager_sleep_failure_response_cb(GtkDialog * dialog, gint response_id,
				      GpmManager * manager)
{
	...
	uri =
	    g_settings_get_string(manager->priv->settings,
				  GPM_SETTINGS_NOTIFY_SLEEP_FAILED_URI);
#if GTK_CHECK_VERSION (3, 22, 0)
	ret =
	    gtk_show_uri_on_window(GTK_WINDOW(dialog), uri,
				   gtk_get_current_event_time(), &error);
#else
	screen = gdk_screen_get_default();
	ret =
	    gtk_show_uri(screen, uri, gtk_get_current_event_time(),
			 &error);
#endif
	if (!ret) {
	    dialog_error =
 ...
 **/
static void
gpm_manager_sleep_failure(GpmManager * manager, gboolean is_suspend,
			  const gchar * detail)
{
    ...
    show_sleep_failed =
	g_settings_get_boolean(manager->priv->settings,
			       GPM_SETTINGS_NOTIFY_SLEEP_FAILED);
    ...
    g_string_append_printf(string, "\n\n%s %s",
			   _("Failure was reported as:"), detail);
	...
	gtk_dialog_add_button(GTK_DIALOG(dialog), _("Visit help page"),
			      GTK_RESPONSE_HELP);
    }
    ...
    g_signal_connect(dialog, "response",
		     G_CALLBACK(gpm_manager_sleep_failure_response_cb),
		     manager);
  out:
    g_free(uri);
...
gpm_manager_action_suspend(GpmManager * manager, const gchar * reason)
{
	...
	if (error) {
	    ...
	    g_string_append(error_message,
			    _
			    ("Suspend failed for some reason (maybe dbus error)"));
	}
...
gpm_manager_action_hibernate(GpmManager * manager, const gchar * reason)
{
	...
	if (error) {
	    ...
	    g_string_append(error_message,
			    _
			    ("Hibernate failed for some reason (maybe dbus error)"));
	}
 ...
 **/
static gboolean
gpm_manager_perform_policy(GpmManager * manager, const gchar * policy_key,
			   const gchar * reason)
{
    GpmActionPolicy policy;

    /* are we inhibited? */
    if (gpm_manager_is_inhibit_valid(manager, FALSE, "policy action") ==
	FALSE)
	return FALSE;
 ...
 **/
static void gpm_manager_idle_do_sleep(GpmManager * manager)
{
    gboolean ret;
    GError *error = NULL;
    GpmActionPolicy policy;
    ...
    if (!manager->priv->on_battery)
	policy =
	    g_settings_get_enum(manager->priv->settings,
				GPM_SETTINGS_ACTION_SLEEP_TYPE_AC);
    else
	policy =
	    g_settings_get_enum(manager->priv->settings,
				GPM_SETTINGS_ACTION_SLEEP_TYPE_BATT);
	...
	if (!ret) {
	    if (error) {	//kobe
		egg_warning
		    ("cannot suspend (error: %s), so trying hibernate",
		     error->message);
		g_error_free(error);
		error = NULL;
	    }
	...
	if (!ret) {
	    if (error) {	//kobe
		egg_warning
		    ("cannot hibernate (error: %s), so trying suspend",
		     error->message);
		g_error_free(error);
		error = NULL;
	    }
 ...
 **/
static void
gpm_manager_idle_changed_cb(GpmIdle * idle, GpmIdleMode mode,
			    GpmManager * manager)
{
...
static void gpm_manager_update_dpms_throttle(GpmManager * manager)
{
	...
	if (manager->priv->screensaver_dpms_throttle_id != 0) {
	    gpm_screensaver_remove_throttle(manager->priv->screensaver,
					    manager->priv->
					    screensaver_dpms_throttle_id);
...
static void
gpm_manager_update_lid_throttle(GpmManager * manager,
 ...
 **/
static void
gpm_manager_button_pressed_cb(GpmButton * button, const gchar * type,
			      GpmManager * manager)
{
    gchar *message;
    ...
    if (g_strcmp0(type, GPM_BUTTON_POWER) == 0 && to_sleep) {
	gpm_manager_perform_policy(manager, GPM_SETTINGS_BUTTON_POWER,
...
#endif
{
    ...
    g_object_get(client,
		 ...
		 "lid-is-closed", &lid_is_closed, NULL);
    //ft拔掉电源判断
    if (on_battery && manager->priv->pre_power_supply_ac) {
	manager->priv->can_suspend = 0;
    } else {
	manager->priv->can_suspend = 1;
    }
    //得到是否可以挂起后重新赋值
    if (on_battery) {
	manager->priv->pre_power_supply_ac = 0;
    }
...
//                gpm_manager_notify_close (manager, manager->priv->notification_below_critical);//kobe
    ...
    event_when_closed =
	g_settings_get_boolean(manager->priv->settings,
			       GPM_SETTINGS_SLEEP_WHEN_CLOSED);
 ...
 **/
static gboolean manager_critical_action_do(GpmManager * manager)
{
    /* stop playing the alert as it's too late to do anything now */
    if (manager->priv->critical_alert_timeout_id)
	gpm_manager_play_loop_stop(manager);

    gpm_manager_perform_policy(manager, GPM_SETTINGS_ACTION_CRITICAL_BATT,
 ...
 */
static void
gpm_manager_engine_icon_changed_cb(GpmEngine * engine, gchar * icon,
				   GpmManager * manager)
{
 ...
 */
static void
gpm_manager_engine_summary_changed_cb(GpmEngine * engine, gchar * summary,
				      GpmManager * manager)
{
 ...
 */
static void
gpm_manager_engine_low_capacity_cb(GpmEngine * engine, UpDevice * device,
				   GpmManager * manager)
{
    gchar *message = NULL;
    const gchar *title;
    ...
    message =
	g_strdup_printf(_
			("Battery has a very low capacity (%1.1f%%), "
 ...
 */
static void
gpm_manager_engine_fully_charged_cb(GpmEngine * engine, UpDevice * device,
				    GpmManager * manager)
{
    UpDeviceKind kind;
    ...
    ret =
	g_settings_get_boolean(manager->priv->settings,
			       GPM_SETTINGS_NOTIFY_FULLY_CHARGED);
    if (!ret) {
    ...
    /* get device properties */
    g_object_get(device, "kind", &kind, "native-path", &native_path, NULL);
...
static void
gpm_manager_engine_close_notify_cb(GpmEngine * engine,
				   GpmManager * manager)
{
    if (gpm_manager_is_inhibit_valid(manager, FALSE, "policy action") ==
	FALSE)
 ...
 */
static void
gpm_manager_engine_discharging_cb(GpmEngine * engine, UpDevice * device,
				  GpmManager * manager)
{
    UpDeviceKind kind;
    gboolean ret;
    ...
    ret =
	g_settings_get_boolean(manager->priv->settings,
			       GPM_SETTINGS_NOTIFY_DISCHARGING);
    if (!ret) {
	egg_debug("no notification");
	goto out;
    }
    ...
    g_object_get(device,
		 ...
		 "time-to-empty", &time_to_empty, NULL);
 ...
 */
static void
gpm_manager_engine_charge_low_cb(GpmEngine * engine, UpDevice * device,
				 GpmManager * manager)
{
	...
	if (!manager->priv->on_battery) {
	    egg_warning
		("ignoring critically low message as we are not on battery power");
	    goto out;
	}
    }
	...
	message =
	    g_strdup_printf(_("Wireless mouse is low in power (%.0f%%)"),
			    percentage);
	...
	/* TRANSLATORS: tell user more details */
	message =
	    g_strdup_printf(_("PDA is low in power (%.0f%%)"), percentage);
	...
	/* TRANSLATORS: tell user more details */
	message =
	    g_strdup_printf(_("Cell phone is low in power (%.0f%%)"),
			    percentage);
	...
	/* TRANSLATORS: tell user more details */
	message =
	    g_strdup_printf(_("Media player is low in power (%.0f%%)"),
			    percentage);
	...
	/* TRANSLATORS: tell user more details */
	message =
	    g_strdup_printf(_("Tablet is low in power (%.0f%%)"),
			    percentage);
 ...
 */
static void
gpm_manager_engine_charge_critical_cb(GpmEngine * engine,
				      UpDevice * device,
				      GpmManager * manager)
{
    const gchar *title = NULL;
	...
	message =
	    g_strdup_printf(_
			    ("Approximately <b>%s</b> of remaining UPS power (%.0f%%). "
 ...
 */
static void
gpm_manager_engine_charge_critical_notify_cb(GpmEngine * engine,
					     UpDevice * device,
					     GpmManager * manager)
{
    const gchar *title = NULL;
	...
	message =
	    g_strdup_printf(_
			    ("Battery power is too low, please plug in the power to ensure the normal use"));
 ...
 */
static void
gpm_manager_engine_charge_action_cb(GpmEngine * engine, UpDevice * device,
				    GpmManager * manager)
{
    const gchar *title = NULL;
    gchar *message = NULL;
	...
	/* use different text for different actions */
	if (policy == GPM_ACTION_POLICY_NOTIFY) {
	    /* TRANSLATORS: computer will shutdown without saving data */
	    message =
		g_strdup(_
			 ("The battery is below the critical level and "
...
//                      egg_debug ("notify========");
	...
	/* wait 5 seconds for user-panic */
	timer_id = g_timeout_add_seconds(10, (GSourceFunc) manager_critical_action_do, manager);	//kobe
	g_source_set_name_by_id(timer_id,
				"[GpmManager] battery critical-action");
	...
	timer_id = g_timeout_add_seconds(10, (GSourceFunc) manager_critical_action_do, manager);	//kobe
	g_source_set_name_by_id(timer_id,
				"[GpmManager] ups critical-action");
...
//                          title, message, GPM_MANAGER_NOTIFY_TIMEOUT_SHORT,
 ...
 **/
static void
gpm_manager_dpms_mode_changed_cb(GpmDpms * dpms, GpmDpmsMode mode,
				 GpmManager * manager)
{
 ...
 */
static gboolean gpm_manager_reset_just_resumed_cb(gpointer user_data)
{
 ...
 **/
static void
gpm_manager_control_resume_cb(GpmControl * control,
			      ...
			      GpmManager * manager)
{
    ...
    timer_id =
	g_timeout_add_seconds(1, gpm_manager_reset_just_resumed_cb,
			      manager);
 ...
 **/
static gint32 gpm_manager_systemd_inhibit(GDBusProxy * proxy)
{
    GError *error = NULL;
    gint32 r = -1;
    gint32 fd = -1;
...
static void gpm_manager_init(GpmManager * manager)
{
    ...
    if (LOGIND_RUNNING()) {
	manager->priv->systemd_inhibit =
	    gpm_manager_systemd_inhibit(manager->priv->
					systemd_inhibit_proxy);
    }
    gpointer arguments;
    GDBusConnection *pconnection;
    pconnection = g_bus_get_sync(G_BUS_TYPE_SYSTEM, NULL, NULL);
    if (pconnection)
...
//      manager->priv->notification_fully_charged = NULL;
    ...
    g_object_get(manager->priv->client,
		 "on-battery", &manager->priv->on_battery, NULL);
    ...
    g_signal_connect(manager->priv->dpms, "mode-changed",
		     G_CALLBACK(gpm_manager_dpms_mode_changed_cb),
		     manager);

    manager->priv->session = gpm_session_new();
	...
	gpm_tray_icon_get_status_icon(manager->priv->tray_icon);
    ...
    g_signal_connect(manager->priv->engine, "icon-changed",
		     G_CALLBACK(gpm_manager_engine_icon_changed_cb),
		     manager);
    g_signal_connect(manager->priv->engine, "summary-changed",
		     G_CALLBACK(gpm_manager_engine_summary_changed_cb),
		     manager);
    g_signal_connect(manager->priv->engine, "fully-charged",
		     G_CALLBACK(gpm_manager_engine_fully_charged_cb),
		     manager);
    g_signal_connect(manager->priv->engine, "discharging",
		     G_CALLBACK(gpm_manager_engine_discharging_cb),
		     manager);
    g_signal_connect(manager->priv->engine, "charge-low",
		     G_CALLBACK(gpm_manager_engine_charge_low_cb),
		     manager);
    g_signal_connect(manager->priv->engine, "charge-critical",
		     G_CALLBACK(gpm_manager_engine_charge_critical_cb),
		     manager);
    g_signal_connect(manager->priv->engine, "charge-action",
		     G_CALLBACK(gpm_manager_engine_charge_action_cb),
		     manager);
    //kobe
    ...
    g_signal_connect(manager->priv->engine, "close-notify",
		     G_CALLBACK(gpm_manager_engine_close_notify_cb),
		     manager);
