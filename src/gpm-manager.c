...
//                gpm_manager_notify_close (manager, manager->priv->notification_below_critical);//kobe
	...
	if (event_when_closed && on_battery && lid_is_closed && manager->priv->can_suspend) {
		gpm_manager_perform_policy (manager, GPM_SETTINGS_BUTTON_LID_BATT,
