...
static void gpm_set_brightness_for_different_mode(GpmTrayIcon *icon,gboolean mode) {
    gchar *command = NULL;
    GError *error = NULL;
    ...
    command = g_strdup_printf ( SBINDIR "/mate-power-backlight-helper --get-max-brightness");
    g_spawn_command_line_sync (command, &stdout_data, NULL, &exit_status, &error);
    value_raw = g_ascii_strtoll(stdout_data,&endptr,10);
    max_brightness=(gint)value_raw;
    g_free (command);
    g_free (stdout_data);

    command = g_strdup_printf ( SBINDIR "/mate-power-backlight-helper --get-brightness");
    g_spawn_command_line_sync (command, &stdout_data, NULL, &exit_status, &error);
    value_raw = g_ascii_strtoll(stdout_data,&endptr,10);
    ...
    if(mode){
	    ...
	    g_settings_set_double (icon->priv->settings,GPM_SETTINGS_BRIGHTNESS_AC,66);
	    ...
	    g_settings_set_double (icon->priv->settings,GPM_SETTINGS_BRIGHTNESS_AC,33);
 ...
 **/
static void
gpm_tray_icon_idle_cb (GpmSession *session, gboolean is_idle, GpmTrayIcon *icon)
{
    gboolean idle_enable = g_settings_get_boolean (icon->priv->settings, GPM_SETTINGS_SWITCH_POLICY);
    if (!idle_enable)	return;
    gboolean ret;
    if(!is_idle)
    {
        ...
        if (mode_choice != 0)
        {
            ...
            g_settings_set_int (icon->priv->settings, GPM_SETTINGS_MODE_POLICY,0);
            mode_choice = 0;
        }
    }
    else
    {
        ...
        if (mode_choice != 2)
        {
            ...
            g_settings_set_int (icon->priv->settings, GPM_SETTINGS_MODE_POLICY,2);
            mode_choice = 2;
        }
    }
}
...
gpm_tray_icon_init (GpmTrayIcon *icon)
{
	...
	gpm_tray_icon_enable_actions (icon, allowed_in_menu);
    	mode_choice = g_settings_get_int (icon->priv->settings, GPM_SETTINGS_MODE_POLICY);


    GError *error = NULL;
    icon->priv->power_mode_proxy = g_dbus_proxy_new_for_bus_sync (G_BUS_TYPE_SYSTEM,
                                                                  G_DBUS_PROXY_FLAGS_DO_NOT_LOAD_PROPERTIES,
                                                                  NULL,
                                                                  "ukui.power.policy",
                                                                  "/ukui/power/policy",
    ...
    gboolean auto_enable = g_settings_get_boolean (icon->priv->settings, GPM_SETTINGS_AUTO_POLICY);
    gpm_tray_icon_policy_enable_auto(icon, auto_enable);
