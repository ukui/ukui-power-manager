 ...
 */
static gint64 egg_idletime_xsyncvalue_to_int64(XSyncValue value)
{
 ...
 */
static void
egg_idletime_xsync_alarm_set(EggIdletime * idletime,
			     ...
			     EggIdletimeAlarmType alarm_type)
{
    ...
    flags =
	XSyncCACounter | XSyncCAValueType | XSyncCATestType | XSyncCAValue
	| XSyncCADelta;
	...
	alarm->xalarm =
	    XSyncCreateAlarm(idletime->priv->dpy, flags, &attr);
}
 ...
 */
static EggIdletimeAlarm *egg_idletime_alarm_find_id(EggIdletime * idletime,
						    guint id)
{
    guint i;
    EggIdletimeAlarm *alarm;
 ...
 */
static void
egg_idletime_set_reset_alarm(EggIdletime * idletime,
			     XSyncAlarmNotifyEvent * alarm_event)
{
	...
	XSyncValueAdd(&alarm->timeout, alarm_event->counter_value, add,
		      &overflow);
 ...
 */
static EggIdletimeAlarm *egg_idletime_alarm_find_event(EggIdletime *
						       idletime,
...
egg_idletime_event_filter_cb(GdkXEvent * gdkxevent, GdkEvent * event,
			     gpointer data)
{
    EggIdletimeAlarm *alarm;
    ...
    /* emit */
    g_signal_emit(alarm->idletime, signals[SIGNAL_ALARM_EXPIRED], 0,
		  alarm->id);
 ...
 */
static EggIdletimeAlarm *egg_idletime_alarm_new(EggIdletime * idletime,
						guint id)
{
    EggIdletimeAlarm *alarm;
 ...
 * egg_idletime_alarm_free:
 */
gboolean egg_idletime_alarm_remove(EggIdletime * idletime, guint id)
{
    EggIdletimeAlarm *alarm;
 ...
 **/
static void egg_idletime_class_init(EggIdletimeClass * klass)
{
 ...
 * egg_idletime_init:
 **/
static void egg_idletime_init(EggIdletime * idletime)
{
    int sync_error;
    int ncounters;
    ...
    /* get the sync event */
    if (!XSyncQueryExtension
	(idletime->priv->dpy, &idletime->priv->sync_event, &sync_error)) {
...
EggIdletime *egg_idletime_new(void)
{
	...
	g_object_add_weak_pointer(egg_idletime_object,
				  &egg_idletime_object);
    }
...
static void egg_test_egg_idletime_wait(guint time_ms)
{
...
static void wait_until_reset(void)
{
    if (last_alarm == 0)
	return;
    g_print("*****************************\n");
...
void egg_idletime_test(gpointer data)
{
	...
	egg_test_failed(test, "time not the same! %i != %i", event_time,
			2000);
    }
	...
	if (event_time > 3000 && event_time < 6000) {
	    egg_test_success(test,
			     "correct, timeout ideally %ims (we did after %ims)",
			     5000, event_time);
	} else {
	    egg_test_failed(test, "alarm %i did not timeout correctly !",
			    last_alarm);
	}
    }
