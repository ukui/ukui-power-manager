 ...
 **/
static gboolean
gcm_backlight_helper_write(const gchar * filename, gint value,
			   GError ** error)
{
    gchar *text = NULL;
    gint retval;
...
gint main(gint argc, gchar * argv[])
{
	...
	{"get-max-brightness", '\0', 0, G_OPTION_ARG_NONE,
	 &get_max_brightness,
	 /* command line argument */
    ...
    g_option_context_set_summary(context,
				 _("UKUI Power Manager Backlight Helper"));
	...
	if (!ret) {
	    /* TRANSLATORS: failed to access backlight file */
	    g_print("%s: %s\n",
		    _("Could not get the maximum value of the backlight"),
		    error->message);
	    g_error_free(error);
	    retval = GCM_BACKLIGHT_HELPER_EXIT_CODE_ARGUMENTS_INVALID;
	    goto out;
	...
	filename_file = g_build_filename(filename, "brightness", NULL);
	ret =
	    gcm_backlight_helper_write(filename_file, set_brightness,
				       &error);
	if (!ret) {
	    /* TRANSLATORS: failed to access backlight file */
	    g_print("%s: %s\n",
