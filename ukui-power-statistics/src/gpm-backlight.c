			     ...
			     GError ** error)
{
    ...
    /* just set the master percentage for now, don't try to be clever */
	    g_message("percentage4=%d",percentage);
    ...
    if (ret && hw_changed) {
	g_message("emitting brightness-changed : %i", percentage);
			    ...
			    GpmBacklight * backlight)
{
	...
	backlight->priv->master_percentage = backlight->priv->master_percentage<=5?5:backlight->priv->master_percentage;
	g_message("backlight->priv->master_percentage ============== %d",backlight->priv->master_percentage);
	if (!on_battery)
				...
				GpmBacklight * backlight)
{
    ...
    gboolean hw_changed;
    gboolean on_battery;
    g_message("Button press event type=%s", type);
	...
	if (ret) {
	    ...
	    g_message("percentage=%d",percentage);
	    backlight->priv->master_percentage = percentage;
...
//                      g_object_get (backlight->priv->client, "on-battery", &on_battery, NULL);
//                      if (!on_battery) {
//                              g_message ("saving brightness for ac supply: %i", percentage);
...
//                      }
	...
	if (ret) {
	    ...
	    gpm_backlight_dialog_show(backlight);
	    /* save the new percentage */
	    g_message("percentage1=%d",percentage);
	    backlight->priv->master_percentage = percentage;
	    g_settings_set_double(backlight->priv->settings,
				  GPM_SETTINGS_BRIGHTNESS_AC,
				  percentage * 1.0);
					 ...
					 gboolean is_idle)
{
    ...
    if (is_idle == FALSE) {
	g_message("we have just been idle for %lfs", elapsed);
	gpm_idle_set_timeout_dim(backlight->priv->idle,
				 backlight->priv->idle_dim_timeout);
	...
	/*if (elapsed < 10) {
	   ...
	   g_message ("increasing idle dim time to %is", backlight->priv->idle_dim_timeout);
	...
	/*if (elapsed > 2*60) {
	   ...
	   g_message ("resetting idle dim time to %is", backlight->priv->idle_dim_timeout);
	   gpm_idle_set_timeout_dim (backlight->priv->idle, backlight->priv->idle_dim_timeout);
	   } */
    } else {
	g_message("we were active for %lfs", elapsed);
    }

    g_message("changing powersave idle status to %i", is_idle);
...
idle_changed_cb(GpmIdle * idle, GpmIdleMode mode, GpmBacklight * backlight)
{
	...
	g_message("ignoring as not on active console");
	return;
    }
...
{
    /* save the new percentage */
	    g_message("percentage3=%d",percentage);
    backlight->priv->master_percentage = percentage;
		  ...
		  GpmBacklight * backlight)
{
    ...
    /* ensure backlight is on */
    ret =
	gpm_dpms_set_mode(backlight->priv->dpms, GPM_DPMS_MODE_ON, &error);
    if (!ret) {
	egg_warning("failed to turn on DPMS: %s", error->message);
	g_error_free(error);
    }
...
static void gpm_backlight_init(GpmBacklight * backlight)
{
    ...
    if (backlight->priv->proxy == NULL) {
	if (error != NULL) {
	    g_message("error connecting to dbus - %s", error->message);
	    g_error_free(error);
	}
    }
}
