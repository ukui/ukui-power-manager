...
gpm_dpms_x11_set_mode(GpmDpms * dpms, GpmDpmsMode mode, GError ** error)
{
	...
	if (3 == mode) {	//add by liyang.zhang
#if 1
	    GPtrArray *argv;
	    gboolean ret9;
	    int r9;
	    argv = g_ptr_array_new();
	    g_ptr_array_add(argv, "/usr/bin/kscreen-doctor");
	    g_ptr_array_add(argv, "--dpms");
	    g_ptr_array_add(argv, "off");
	    g_ptr_array_add(argv, NULL);
	    gchar **env = g_get_environ();
	    env =
		g_environ_setenv(env, "QT_QPA_PLATFORM", "wayland", TRUE);
	    ret9 =
		g_spawn_sync(NULL, (char **) argv->pdata, env,
 ...
 **/
static gboolean gpm_dpms_poll_mode_cb(GpmDpms * dpms)
{
    gboolean ret;
    GpmDpmsMode mode;
    GError *error = NULL;
 ...
 **/
static gboolean gpm_dpms_clear_timeouts(GpmDpms * dpms)
{
    gboolean ret = FALSE;
...
static void gpm_dpms_init(GpmDpms * dpms)
{
    ...
    dpms->priv->timer_id =
	g_timeout_add_seconds(GPM_DPMS_POLL_TIME,
