 ...
 **/
static const gchar *gpm_idle_mode_to_string(GpmIdleMode mode)
{
 ...
 **/
static void gpm_idle_set_mode(GpmIdle * idle, GpmIdleMode mode)
{
    g_return_if_fail(GPM_IS_IDLE(idle));
    g_message("Doing a state transition: %s",
	      gpm_idle_mode_to_string(mode));
    if (mode != idle->priv->mode) {
	idle->priv->mode = mode;
	egg_debug("Doing a state transition: %s",
		  gpm_idle_mode_to_string(mode));
	g_message("done a state transition: %s",
		  gpm_idle_mode_to_string(mode));
 ...
 **/
void gpm_idle_set_check_cpu(GpmIdle * idle, gboolean check_type_cpu)
{
    g_return_if_fail(GPM_IS_IDLE(idle));
 ...
 * gpm_idle_blank_cb:
 **/
static gboolean gpm_idle_blank_cb(GpmIdle * idle)
{
    if (idle->priv->mode > GPM_IDLE_MODE_BLANK) {
	egg_debug("ignoring current mode %s",
		  gpm_idle_mode_to_string(idle->priv->mode));
	g_message("ignoring current mode %s",
		  gpm_idle_mode_to_string(idle->priv->mode));
	return FALSE;
    }
 ...
 * gpm_idle_sleep_cb:
 **/
static gboolean gpm_idle_sleep_cb(GpmIdle * idle)
{
    gdouble load;
    gboolean ret = FALSE;
 ...
 * gpm_idle_evaluate:
 **/
static void gpm_idle_evaluate(GpmIdle * idle)
{
    gboolean is_lock;
    gboolean is_idle;
    ...
    is_lock = gpm_session_get_lock(idle->priv->session);
    is_idle = gpm_session_get_idle(idle->priv->session);
    is_idle_inhibited =
	gpm_session_get_idle_inhibited(idle->priv->session);
    is_suspend_inhibited =
	gpm_session_get_suspend_inhibited(idle->priv->session);
    egg_debug
	("session_idle=%i, idle_inhibited=%i, suspend_inhibited=%i, x_idle=%i",
	 is_idle, is_idle_inhibited, is_suspend_inhibited,
	 idle->priv->x_idle);
    ...
    /* normal to dim */
    if (idle->priv->mode == GPM_IDLE_MODE_NORMAL && idle->priv->dim_idle) {
...
static void gpm_idle_control(GpmIdle * idle)
{
    gboolean is_lock;
    gboolean is_idle;
    gboolean is_idle_inhibited;
	...
	guint timeout_blank = idle->priv->timeout_blank - 60;
	if (timeout_blank <= 0)
	    timeout_blank = 1;
	idle->priv->timeout_blank_id = g_timeout_add_seconds(timeout_blank,
							     (GSourceFunc)
							     gpm_idle_blank_cb,
							     idle);
	g_source_set_name_by_id(idle->priv->timeout_blank_id,
				"[GpmIdle] blank");
    }
	...
	if (idle->priv->timeout_sleep_id == 0 &&
	    ...
	    guint timeout_sleep = idle->priv->timeout_sleep - 60;
	    if (timeout_sleep <= 0)
		timeout_sleep = 1;
	    idle->priv->timeout_sleep_id =
		g_timeout_add_seconds(timeout_sleep,
				      (GSourceFunc) gpm_idle_sleep_cb,
				      idle);
	    g_source_set_name_by_id(idle->priv->timeout_sleep_id,
				    "[GpmIdle] sleep");
	}
    }
 ...
 **/
static guint gpm_idle_adjust_timeout_dim(guint idle_time, guint timeout)
{
 ...
 **/
gboolean gpm_idle_set_timeout_dim(GpmIdle * idle, guint timeout)
{
    ...
    timeout_adjusted =
	gpm_idle_adjust_timeout_dim(idle_time_in_msec / 1000, timeout);
    egg_debug
	("Current idle time=%lldms, timeout was %us, becomes %us after adjustment",
	...
	if (timeout > 0)
	    egg_idletime_alarm_set(idle->priv->idletime,
				   GPM_IDLE_IDLETIME_ID, timeout * 1000);
	else
	    egg_idletime_alarm_remove(idle->priv->idletime,
				      GPM_IDLE_IDLETIME_ID);
    }
    return TRUE;
}
 ...
 * @timeout: The new timeout we want to set, in seconds
 **/
gboolean gpm_idle_set_timeout_blank(GpmIdle * idle, guint timeout)
{
    g_return_val_if_fail(GPM_IS_IDLE(idle), FALSE);
 ...
 * @timeout: The new timeout we want to set, in seconds
 **/
gboolean gpm_idle_set_timeout_sleep(GpmIdle * idle, guint timeout)
{
    g_return_val_if_fail(GPM_IS_IDLE(idle), FALSE);
 ...
 **/
static void
gpm_idle_session_idle_changed_cb(GpmSession * session, gboolean is_idle,
				 GpmIdle * idle)
{
...
static void
gpm_idle_session_control_changed_cb(GpmSession * session, gboolean is_idle,
				    GpmIdle * idle)
{
 ...
 **/
static void
gpm_idle_session_inhibited_changed_cb(GpmSession * session,
				      gboolean is_idle_inhibited,
				      gboolean is_suspend_inhibited,
				      GpmIdle * idle)
{
    egg_debug
	("Received ukui session inhibited changed: idle=(%i), suspend=(%i)",
	 is_idle_inhibited, is_suspend_inhibited);
//      gpm_idle_evaluate (idle);
    gpm_idle_control(idle);
 ...
 **/
static void
gpm_idle_idletime_alarm_expired_cb(EggIdletime * idletime, guint alarm_id,
				   GpmIdle * idle)
{
...
static void gpm_idle_init(GpmIdle * idle)
{
    ...
    idle->priv->x_idle = FALSE;
    idle->priv->dim_idle = FALSE;
    idle->priv->control = FALSE;
    idle->priv->load = gpm_load_new();
    idle->priv->session = gpm_session_new();
    g_signal_connect(idle->priv->session, "idle-changed",
		     G_CALLBACK(gpm_idle_session_idle_changed_cb), idle);
    g_signal_connect(idle->priv->session, "control-changed",
		     G_CALLBACK(gpm_idle_session_control_changed_cb),
		     idle);
    g_signal_connect(idle->priv->session, "inhibited-changed",
		     G_CALLBACK(gpm_idle_session_inhibited_changed_cb),
		     idle);

    idle->priv->idletime = egg_idletime_new();
    g_signal_connect(idle->priv->idletime, "reset",
		     G_CALLBACK(gpm_idle_idletime_reset_cb), idle);
    g_signal_connect(idle->priv->idletime, "alarm-expired",
		     G_CALLBACK(gpm_idle_idletime_alarm_expired_cb), idle);
...
static void
gpm_idle_test_idle_changed_cb(GpmIdle * idle, GpmIdleMode mode,
			      EggTest * test)
{
    _mode = mode;
...
static gboolean gpm_idle_test_delay_cb(EggTest * test)
{
