 ...
 */
static void
gpm_phone_battery_state_changed(DBusGProxy * proxy, guint idx,
				guint percentage, gboolean on_ac,
				GpmPhone * phone)
{
    g_return_if_fail(GPM_IS_PHONE(phone));

    egg_debug("got BatteryStateChanged %i = %i (%i)", idx, percentage,
	      on_ac);
 ...
 */
static void
gpm_phone_num_batteries_changed(DBusGProxy * proxy, guint number,
				GpmPhone * phone)
{
    g_return_if_fail(GPM_IS_PHONE(phone));
			      ...
			      GpmPhone * phone)
{
	...
	if (error != NULL) {
	    egg_warning("Could not connect to DBUS daemon: %s",
			error->message);
	    g_error_free(error);
	...
	g_clear_error(&error);
	phone->priv->proxy =
	    dbus_g_proxy_new_for_name_owner(phone->priv->connection,
	...
	if (error != NULL) {
	    egg_warning
		("Cannot connect, maybe the daemon is not running: %s",
		 error->message);
	    g_error_free(error);
	...
	dbus_g_object_register_marshaller
	    (gpm_marshal_VOID__UINT_UINT_BOOLEAN, G_TYPE_NONE, G_TYPE_UINT,
	     G_TYPE_UINT, G_TYPE_BOOLEAN, G_TYPE_INVALID);
	...
	dbus_g_proxy_add_signal(phone->priv->proxy, "BatteryStateChanged",
				...
				G_TYPE_INVALID);
	dbus_g_proxy_connect_signal(phone->priv->proxy,
				    "BatteryStateChanged",
				    G_CALLBACK
	...
	dbus_g_proxy_add_signal(phone->priv->proxy,
				"NumberBatteriesChanged", G_TYPE_UINT,
				G_TYPE_INVALID);
	dbus_g_proxy_connect_signal(phone->priv->proxy,
				    "NumberBatteriesChanged",
				    G_CALLBACK
...
gpm_phone_service_vanished_cb(GDBusConnection * connection,
			      const gchar * name, GpmPhone * phone)
{
    g_return_if_fail(GPM_IS_PHONE(phone));
...
static void egg_test_mainloop_wait(guint ms)
{
    GMainLoop *loop;
...
void gpm_phone_test(gpointer data)
{
    ...
    g_signal_connect(phone, "device-refresh",
		     G_CALLBACK(phone_device_refresh_cb),
		     GUINT_TO_POINTER(44));
	...
	egg_test_failed(test,
			"not connected with %i (phone not connected?)",
			value);
    }
    ...
    if (value != 0) {
	egg_test_success(test, "percentage is %i",
			 phone->priv->percentage);
    } else {
