...
void gpm_prefs_activate_window(GtkApplication * app, GpmPrefs * prefs)
{
	...
	GTK_WINDOW(gtk_builder_get_object
		   (prefs->priv->builder, "dialog_preferences"));
 ...
 * @prefs: This prefs class instance
 **/
static void gpm_prefs_help_cb(GtkWidget * widget, GpmPrefs * prefs)
{
 ...
 * @widget: The GtkWidget object
 **/
static void gpm_prefs_icon_radio_cb(GtkWidget * widget, GpmPrefs * prefs)
{
    gint policy;

    policy =
	GPOINTER_TO_INT(g_object_get_data(G_OBJECT(widget), "policy"));
    g_settings_set_enum(prefs->priv->settings, GPM_SETTINGS_ICON_POLICY,
			policy);
}
 ...
 * @value: The value in %.
 **/
static gchar *gpm_prefs_format_percentage_cb(GtkScale * scale,
					     gdouble value)
{
...
gpm_prefs_action_combo_changed_cb(GtkWidget * widget, GpmPrefs * prefs)
{
    ...
    actions =
	(const GpmActionPolicy *) g_object_get_data(G_OBJECT(widget),
						    "actions");
    gpm_pref_key =
 ...
 **/
static void gpm_prefs_actions_destroy_cb(GpmActionPolicy * array)
{
    g_free(array);
}
			     ...
			     const GpmActionPolicy * actions)
{
	...
	GTK_WIDGET(gtk_builder_get_object
		   (prefs->priv->builder, widget_name));

    value = g_settings_get_enum(prefs->priv->settings, gpm_pref_key);
    is_writable =
	g_settings_is_writable(prefs->priv->settings, gpm_pref_key);

    gtk_widget_set_sensitive(widget, is_writable);

    array = g_ptr_array_new();
    g_object_set_data(G_OBJECT(widget), "settings_key",
    ...
    g_object_set_data_full(G_OBJECT(widget), "actions",
			   (gpointer) actions_added,
 ...
 * @prefs: This prefs class instance
 **/
static void gpm_prefs_close_cb(GtkWidget * widget, GpmPrefs * prefs)
{
 ...
 **/
static gboolean
gpm_prefs_delete_event_cb(GtkWidget * widget, GdkEvent * event,
			  GpmPrefs * prefs)
{
...
static void prefs_setup_notification(GpmPrefs * prefs)
{
    gint icon_policy;
    ...
    icon_policy =
	g_settings_get_enum(prefs->priv->settings,
			    GPM_SETTINGS_ICON_POLICY);
    ...
    radiobutton_icon_low =
	GTK_WIDGET(gtk_builder_get_object
		   (prefs->priv->builder, "radiobutton_notification_low"));
...
static void prefs_setup_ac(GpmPrefs * prefs)
{
    GtkWidget *widget;
    ...
    static const gint computer_times[] = { 10 * 60,
	30 * 60,
	1 * 60 * 60,
	2 * 60 * 60,
	0,			/* never */
    ...
    static const gint display_times[] = { 1 * 60,
	5 * 60,
	10 * 60,
	30 * 60,
	1 * 60 * 60,
    ...
    widget =
	GTK_WIDGET(gtk_builder_get_object
		   (prefs->priv->builder, "hscale_ac_brightness"));
    ...
    widget =
	GTK_WIDGET(gtk_builder_get_object
		   (prefs->priv->builder, "checkbutton_ac_display_dim"));
	...
	widget =
	    GTK_WIDGET(gtk_builder_get_object
		       (prefs->priv->builder, "hbox_ac_lid"));
	gtk_widget_hide(widget);
    }
    if (prefs->priv->has_lcd == FALSE) {
	widget =
	    GTK_WIDGET(gtk_builder_get_object
		       (prefs->priv->builder, "hbox_ac_brightness"));
	...
	widget =
	    GTK_WIDGET(gtk_builder_get_object
		       (prefs->priv->builder,
			"checkbutton_ac_display_dim"));

	gtk_widget_hide(widget);
    }
}

static void prefs_setup_battery(GpmPrefs * prefs)
{
    GtkWidget *widget;
	...
	notebook =
	    GTK_NOTEBOOK(gtk_builder_get_object
			 (prefs->priv->builder, "notebook_preferences"));
	widget =
	    GTK_WIDGET(gtk_builder_get_object
		       (prefs->priv->builder, "vbox_battery"));
    ...
    widget =
	GTK_WIDGET(gtk_builder_get_object
		   (prefs->priv->builder, "hscale_bat_brightness"));
    g_settings_bind(prefs->priv->settings, GPM_SETTINGS_BRIGHTNESS_BAT,
		    gtk_range_get_adjustment(GTK_RANGE(widget)), "value",
		    G_SETTINGS_BIND_DEFAULT);
    g_signal_connect(G_OBJECT(widget), "format-value",
		     G_CALLBACK(gpm_prefs_format_percentage_cb), NULL);
	...
	GTK_WIDGET(gtk_builder_get_object
		   (prefs->priv->builder,
		    "checkbutton_battery_display_reduce"));
    g_settings_bind(prefs->priv->settings,
		    GPM_SETTINGS_BACKLIGHT_BATTERY_REDUCE, widget,
		    "active", G_SETTINGS_BIND_DEFAULT);
    widget =
	GTK_WIDGET(gtk_builder_get_object
		   (prefs->priv->builder,
		    "checkbutton_battery_display_dim"));
	...
	widget =
	    GTK_WIDGET(gtk_builder_get_object
		       (prefs->priv->builder, "hbox_battery_lid"));
...
static void prefs_setup_ups(GpmPrefs * prefs)
{
    GtkWidget *widget;
    GtkNotebook *notebook;
    gint page;

    const GpmActionPolicy ups_low_actions[] = { GPM_ACTION_POLICY_NOTHING,
	GPM_ACTION_POLICY_HIBERNATE,
	GPM_ACTION_POLICY_SHUTDOWN,
	-1
    };
...
static void prefs_setup_general(GpmPrefs * prefs)
{
    GtkWidget *widget;
	...
	widget =
	    GTK_WIDGET(gtk_builder_get_object
		       (prefs->priv->builder, "hbox_general_suspend"));
...
static void gpm_prefs_init(GpmPrefs * prefs)
{
	...
	prefs->priv->has_button_lid =
	    up_client_get_lid_is_present(prefs->priv->client);
    }
    ...
    /* get device list */
    ret =
	up_client_enumerate_devices_sync(prefs->priv->client, NULL,
					 &error);
    if (!ret) {
    ...
    retval =
	gtk_builder_add_from_file(prefs->priv->builder,
				  GPM_DATA "/gpm-prefs.ui", &error);
    ...
    widget =
	GTK_WIDGET(gtk_builder_get_object
		   (prefs->priv->builder, "button_close"));
    g_signal_connect(widget, "clicked", G_CALLBACK(gpm_prefs_close_cb),
		     prefs);

    widget =
	GTK_WIDGET(gtk_builder_get_object
		   (prefs->priv->builder, "button_help"));
    g_signal_connect(widget, "clicked", G_CALLBACK(gpm_prefs_help_cb),
		     prefs);

    widget =
	GTK_WIDGET(gtk_builder_get_object
		   (prefs->priv->builder, "button_defaults"));
    gtk_widget_hide(widget);
