...
typedef enum {
    ...
    GPM_SESSION_STATUS_ENUM_PAD,
    GPM_SESSION_STATUS_ENUM_IDLE_CONTROL,
...
//            session->priv->is_lock = (status == GPM_SESSION_STATUS_ENUM_IDLE);
	...
	if (status == GPM_SESSION_STATUS_ENUM_AVAILABLE) {
	    is_idle = FALSE;
//              if (is_idle != session->priv->is_idle_old) {
...
//            session->priv->is_lock = (status == GPM_SESSION_STATUS_ENUM_IDLE);
	...
	//added by jh
	if (status == GPM_SESSION_STATUS_ENUM_IDLE_CONTROL) {
	    control = TRUE;
	    g_signal_emit(session, signals[CONTROL_CHANGED], 0, control);
	}
	if (status == GPM_SESSION_STATUS_ENUM_AVAILABLE) {
	    control = FALSE;
	    g_signal_emit(session, signals[CONTROL_CHANGED], 0, control);
 ...
 * gpm_session_is_idle:
 **/
static gboolean gpm_session_is_idle(GpmSession * session)
{
    gboolean ret;
 ...
 **/
static gboolean gpm_session_is_idle_inhibited(GpmSession * session)
{
    gboolean ret;
 ...
 **/
static gboolean gpm_session_is_suspend_inhibited(GpmSession * session)
{
    gboolean ret;
    gboolean is_inhibited = FALSE;
    GError *error = NULL;
    ...
    u_cookie = g_dbus_proxy_call_sync(session->priv->proxy,
				      "IsInhibited",
				      g_variant_new("(u)",
						    GPM_SESSION_INHIBIT_MASK_SUSPEND),
				      G_DBUS_CALL_FLAGS_NONE, -1, NULL,
				      &error);
 ...
 **/
static void
gpm_session_stop_cb(GDBusProxy * proxy, gchar * sender_name,
 ...
 **/
static void
gpm_session_query_end_session_cb(GDBusProxy * proxy, gchar * sender_name,
				 gchar * signal_name,
 ...
 **/
static void
gpm_session_end_session_cb(GDBusProxy * proxy, gchar * sender_name,
			   gchar * signal_name, GVariant * parameters,
			   gpointer user_data)
{
    GpmSession *session = GPM_SESSION(user_data);
 ...
 **/
gboolean
gpm_session_end_session_response(GpmSession * session, gboolean is_okay,
				 const gchar * reason)
{
    gboolean ret = FALSE;
    GError *error = NULL;
    GVariant *u_cookie;
    g_return_val_if_fail(GPM_IS_SESSION(session), FALSE);
    g_return_val_if_fail(session->priv->proxy_client_private != NULL,
			 FALSE);
 ...
 **/
gboolean
gpm_session_register_client(GpmSession * session, const gchar * app_id,
			    const gchar * client_startup_id)
{
    ...
    session->priv->proxy_client_private =
	g_dbus_proxy_new_for_bus_sync(G_BUS_TYPE_SESSION,
				      G_DBUS_PROXY_FLAGS_DO_NOT_LOAD_PROPERTIES,
				      NULL, GPM_SESSION_MANAGER_SERVICE,
				      client_id,
    ...
    g_signal_connect(session->priv->proxy_client_private, "g-signal",
		     G_CALLBACK(gpm_session_stop_cb), session);

    /* get QueryEndSession */

    g_signal_connect(session->priv->proxy_client_private,
		     "QueryEndSession",
		     G_CALLBACK(gpm_session_query_end_session_cb),
		     session);
    /* get EndSession */

    g_signal_connect(session->priv->proxy_client_private, "EndSession",
		     G_CALLBACK(gpm_session_end_session_cb), session);
 ...
 **/
static void
gpm_session_inhibit_changed_cb(GDBusProxy * proxy, gchar * sender_name,
			       gchar * signal_name, GVariant * parameters,
			       gpointer user_data)
{
	...
	|| (g_strcmp0(signal_name, "InhibitorAdded") == 0)) {

	is_idle_inhibited = gpm_session_is_idle_inhibited(session);
	is_suspend_inhibited = gpm_session_is_suspend_inhibited(session);
	if (is_idle_inhibited != session->priv->is_idle_inhibited_old
	    || is_suspend_inhibited !=
	    session->priv->is_suspend_inhibited_old) {
	    egg_debug
		("emitting inhibited-changed : idle=(%i), suspend=(%i)",
	    ...
	    g_signal_emit(session, signals[INHIBITED_CHANGED], 0,
			  is_idle_inhibited, is_suspend_inhibited);
	}
    }
}
 ...
 **/
static void gpm_session_class_init(GpmSessionClass * klass)
{
	...
	g_signal_new("cancel-end-session",
		     ...
		     NULL, NULL, g_cclosure_marshal_VOID__VOID,
		     G_TYPE_NONE, 0);
    signals[CONTROL_CHANGED] =
	g_signal_new("control-changed",
		     G_TYPE_FROM_CLASS(object_class),
		     G_SIGNAL_RUN_LAST,
		     G_STRUCT_OFFSET(GpmSessionClass, control_changed),
		     NULL, NULL, g_cclosure_marshal_VOID__BOOLEAN,
		     G_TYPE_NONE, 1, G_TYPE_BOOLEAN);
}
 ...
 **/
static void gpm_session_init(GpmSession * session)
{
//      DBusGConnection *connection;
    GError *error = NULL;
    ...
    session->priv->proxy_prop =
	g_dbus_proxy_new_for_bus_sync(G_BUS_TYPE_SESSION,
				      G_DBUS_PROXY_FLAGS_DO_NOT_LOAD_PROPERTIES,
				      NULL, GPM_SESSION_MANAGER_SERVICE,
				      GPM_SESSION_MANAGER_PRESENCE_PATH,
				      GPM_DBUS_PROPERTIES_INTERFACE, NULL,
				      &error);
    ...
    g_signal_connect(session->priv->proxy_presence, "g-signal",
		     G_CALLBACK(gpm_session_presence_status_changed_cb),
		     session);
    ...
    /* coldplug */
    session->priv->is_idle_inhibited_old =
	gpm_session_is_idle_inhibited(session);
    session->priv->is_suspend_inhibited_old =
	gpm_session_is_suspend_inhibited(session);
    session->priv->is_idle_old = gpm_session_is_idle(session);
    egg_debug("idle: %i, idle_inhibited: %i, suspend_inhibited: %i",
	      session->priv->is_idle_old,
	      session->priv->is_idle_inhibited_old,
	      session->priv->is_suspend_inhibited_old);
}
