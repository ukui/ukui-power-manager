 ...
 **/
static void gpm_tray_icon_show(GpmTrayIcon * icon, gboolean enabled)
{
    g_return_if_fail(GPM_IS_TRAY_ICON(icon));
 ...
 **/
static void gpm_tray_icon_show_info_cb(GtkMenuItem * item, gpointer data)
{
    gchar *path;
    ...
    path =
	g_strdup_printf("%s/ukui-power-statistics --device %s", BINDIR,
			object_path);
...
//kobe
static gboolean
gpm_tray_icon_show_info_cb_ukui(GtkWidget * w, GdkEventButton * e,
				GpmTrayIcon * icon)
{
    gchar *path;
    const gchar *object_path;
...
//jiangh powerpolicy
static gboolean gpm_tray_icon_power_policy(gint mode)
{
    GError *error = NULL;
    GDBusProxy *proxy;
    GVariant *res = NULL;
...
/**
 * gpm_tray_icon_policy_enable_auto:
 **/
static gboolean
gpm_tray_icon_policy_enable_auto(GpmTrayIcon * icon, gboolean enable)
{
    g_return_if_fail(GPM_IS_TRAY_ICON(icon));
    GError *error = NULL;
    GVariant *res = NULL;
    GDBusProxy *proxy = icon->priv->power_mode_proxy;
    if (proxy == NULL)
	return FALSE;
    res = g_dbus_proxy_call_sync(proxy, "change_auto_switch",
...
static gboolean
gpm_tray_icon_power_mode_restore(GpmTrayIcon * icon, gint mode)
{
    GError *error = NULL;
    GVariant *res = NULL;
    GDBusProxy *proxy = icon->priv->power_mode_proxy;
...
/**
 * gpm_tray_icon_show_perf_cb:
 * callback function for performance button pressed
 **/
static void gpm_tray_icon_show_perf_cb(GtkMenuItem * item, gpointer data)
{
    gpm_tray_icon_power_policy(0);
}

static void gpm_set_brightness_for_different_mode(GpmTrayIcon * icon,
						  gboolean mode)
{
    gchar *command = NULL;
    GError *error = NULL;
    ...
    command =
	g_strdup_printf(SBINDIR
			"/mate-power-backlight-helper --get-max-brightness");
    g_spawn_command_line_sync(command, &stdout_data, NULL, &exit_status,
			      &error);
    value_raw = g_ascii_strtoll(stdout_data, &endptr, 10);
    max_brightness = (gint) value_raw;
    g_free(command);
    g_free(stdout_data);

    command =
	g_strdup_printf(SBINDIR
			"/mate-power-backlight-helper --get-brightness");
    g_spawn_command_line_sync(command, &stdout_data, NULL, &exit_status,
			      &error);
    value_raw = g_ascii_strtoll(stdout_data, &endptr, 10);
    current_brightness = (gint) value_raw;

    gint value = max_brightness / 3;


    if (mode) {
	if (current_brightness < value * 2) {
	    current_brightness = value * 2;
	    g_settings_set_double(icon->priv->settings,
				  GPM_SETTINGS_BRIGHTNESS_AC, 66);
...
/**
 * gpm_tray_icon_show_save_cb:
 * @action: A valid GtkAction
 **/
static void gpm_tray_icon_show_save_cb(GtkMenuItem * item, gpointer data)
{
    ...
    g_settings_set_int(icon->priv->settings, GPM_SETTINGS_MODE_POLICY,
		       MODE_SAVE);
 ...
 **/
static void
gpm_tray_icon_show_performance_cb(GtkMenuItem * item, gpointer data)
{

    GpmTrayIcon *icon = GPM_TRAY_ICON(data);
    gpm_set_brightness_for_different_mode(icon, 1);
    ...
    gtk_show_about_dialog(NULL,
			  ...
			  "copyright",
			  _
			  ("Copyright \xC2\xA9 2011-2017 UKUI developers"),
			  "authors", authors,
			  ...
			  "website", "http://www.ukui.org", NULL);
}
 ...
 **/
static guint
gpm_tray_icon_add_device(GpmTrayIcon * icon, GtkMenu * menu,
			 const GPtrArray * array, UpDeviceKind kind)
{
    guint i;
    guint added = 0;
    gchar *icon_name;
	...
	g_object_get(device,
		     ...
		     "vendor", &vendor, "model", &model, NULL);

	if (kind_tmp == UP_DEVICE_KIND_BATTERY) {
	    icon->priv->has_batteries = TRUE;
	}
	...
	if ((vendor != NULL && strlen(vendor) != 0)
	    ...
	    label =
		g_strdup_printf("%s %s (%.1f%%)", vendor, model,
				percentage);
	} else {
	    label =
		g_strdup_printf("%s (%.1f%%)",
				gpm_device_kind_to_localised_string(kind,
								    1),
				percentage);
	}
	...
	g_object_set_data(G_OBJECT(item), "object-path",
...
//kobe
static guint
gpm_tray_icon_add_device_ukui(GpmTrayIcon * icon, GtkWidget * vbox,
			      const GPtrArray * array, UpDeviceKind kind)
{
    guint i;
    guint added = 0;
	...
	g_object_get(device,
		     "kind", &kind_tmp,
		     "state", &state, "percentage", &percentage, NULL);
	...
	if (is_charging)
	    text =
		g_strdup_printf(_
				("%.1f%% available power\n(The power is connected and is charging)"),
				percentage);
	else
	    text =
		g_strdup_printf(_("%.1f%% available power"), percentage);
	...
	g_object_set_data(event_box, "object-path",
			  (gpointer) object_path);
	g_signal_connect(event_box, "button-press-event",
			 G_CALLBACK(gpm_tray_icon_show_info_cb_ukui),
			 icon);
...
//kobe
static
void on_menu_pos(GtkMenu * menu, gint * x, gint * y, gboolean * push_in,
		 GtkStatusIcon * icon)
{
    GdkRectangle rect;
    ...
    if (gtk_status_icon_get_geometry(icon, &screen, &rect, &orientation) ==
	FALSE)
	goto out;
	...
	if (rect.y + rect.height + menu_req.height <=
	    monitor_rect.y + monitor_rect.height)
	    menu_y = monitor_rect.y + panel_height;
	else
	    menu_y =
		monitor_rect.y + monitor_rect.height - panel_height -
		menu_req.height;
 ...
 **/
static void
gpm_tray_icon_add_primary_device(GpmTrayIcon * icon, GtkMenu * menu,
				 UpDevice * device)
{
    GtkWidget *item;
    gchar *time_str;
    ...
    /* get details */
    g_object_get(device, "time-to-empty", &time_to_empty, NULL);
...
static char *gpm_tray_caculate_theme(GtkWidget * widget)
{
    ...
    sprintf(color_hex, "\#%s%s%s", color_hex_red, color_hex_green,
	    color_hex_blue);
//    g_message("colorhex:%s----------------",color_hex);
    char *script = "* {"
	"color: #f2f2f2;" "background-color: #283138;" "opacity: 0.8;" "}";
    if (!strcmp(color_hex, "#283138")) {
	script =
	    "* {"
	    "color: #f2f2f2;"
	    "background-color: #283138;" "opacity: 0.7;" "}";
 ...
 **/
static GtkMenu *gpm_tray_icon_create_menu(GpmTrayIcon * icon)
{
    ...
    dev_cnt +=
	gpm_tray_icon_add_device(icon, menu, array,
				 UP_DEVICE_KIND_BATTERY);
    dev_cnt +=
	gpm_tray_icon_add_device(icon, menu, array, UP_DEVICE_KIND_UPS);
    dev_cnt +=
	gpm_tray_icon_add_device(icon, menu, array, UP_DEVICE_KIND_MOUSE);
    dev_cnt +=
	gpm_tray_icon_add_device(icon, menu, array,
				 UP_DEVICE_KIND_KEYBOARD);
    dev_cnt +=
	gpm_tray_icon_add_device(icon, menu, array, UP_DEVICE_KIND_PDA);
    dev_cnt +=
	gpm_tray_icon_add_device(icon, menu, array, UP_DEVICE_KIND_PHONE);
    dev_cnt +=
	gpm_tray_icon_add_device(icon, menu, array,
				 UP_DEVICE_KIND_MEDIA_PLAYER);
    dev_cnt +=
	gpm_tray_icon_add_device(icon, menu, array, UP_DEVICE_KIND_TABLET);
    dev_cnt +=
	gpm_tray_icon_add_device(icon, menu, array,
 ...
 **/
static void gpm_tray_icon_popup_menu(GpmTrayIcon * icon, guint32 timestamp)
{
    GtkMenu *menu;
 ...
 **/
static void
gpm_tray_icon_popup_menu_cb(GtkStatusIcon * status_icon, guint button,
			    guint32 timestamp, GpmTrayIcon * icon)
{
...
gpm_tray_icon_add_vbox_content(GpmTrayIcon * icon, GtkWidget * vbox)
{
    ...
    array = gpm_engine_get_devices(icon->priv->engine);
    dev_cnt +=
	gpm_tray_icon_add_device_ukui(icon, vbox, array,
				      UP_DEVICE_KIND_BATTERY);
...
void draw_pref_button(GtkWidget * widget, cairo_t * cr,
		      cairo_surface_t * img)
{
    ...
    cairo_rectangle(cr, alloc.x + 0.5, alloc.y + 1.5, alloc.width - 0.5,
		    alloc.height - 1.5);
    cairo_fill(cr);
    ...
    cairo_move_to(cr, alloc.x + (alloc.width - width) / 2,
...
//kobe
static gboolean
on_button_press(GtkWidget * button, GdkEventButton * event,
		gpointer user_data)
{
    if (event->button == 1) {
	button_pressed = TRUE;
...
//kobe
static gboolean
on_button_release(GtkWidget * button, GdkEventButton * event,
		  gpointer user_data)
{
    if (event->button == 1) {
...
//kobe
GdkColor get_border_color(GtkWidget * w, GpmTrayIcon * icon,
			  char *color_name)
{
    GdkColor color;

    GtkSettings *gs =
	gtk_settings_get_for_screen(gtk_status_icon_get_screen
				    (icon->priv->status_icon));
...
/**
 * gpm_tray_icon_on_power_policy_signal:
 **/
static void
gpm_tray_icon_on_power_policy_signal(GDBusProxy * proxy,
				     gchar * sender_name,
				     gchar * signal_name,
				     GVariant * parameters,
				     gpointer user_data)
{
    guint value;
    GpmTrayIcon *icon = GPM_TRAY_ICON(user_data);

    if (g_strcmp0(signal_name, "ModeChanged") == 0) {
	g_variant_get(parameters, "(i)", &value);
	mode_choice = value;
	if (value == MODE_PERFORMANCE)
	    gpm_set_brightness_for_different_mode(icon, 1);
				  ...
				  GpmTrayIcon * icon)
{
    ...
    } else if (g_strcmp0(key, GPM_SETTINGS_AUTO_POLICY) == 0) {
	...
	{
	    gboolean ret = gpm_tray_icon_power_policy(MODE_PERFORMANCE);
	    g_settings_set_int(icon->priv->settings,
			       GPM_SETTINGS_MODE_POLICY, MODE_PERFORMANCE);
	    mode_choice = MODE_PERFORMANCE;
	} else if (cur_md == 2)	//节能模式
	{
	    gboolean ret = gpm_tray_icon_power_policy(MODE_SAVE);
	    g_settings_set_int(icon->priv->settings,
			       GPM_SETTINGS_MODE_POLICY, MODE_SAVE);
	    mode_choice = MODE_SAVE;
	}
    }
}
...
/**
 * gpm_tray_icon_idle_cb:
 *
 * do power policy when session emit change signal; do them here.
 **/
static void
gpm_tray_icon_idle_cb(GpmSession * session, gboolean is_idle,
		      GpmTrayIcon * icon)
{
    gboolean idle_enable =
	g_settings_get_boolean(icon->priv->settings,
			       GPM_SETTINGS_SWITCH_POLICY);
    if (!idle_enable)
	return;
    gboolean ret;
    if (!is_idle) {
	...
	if (mode_choice != 0) {
	    ...
	    ret = gpm_tray_icon_power_policy(0);
	    g_settings_set_int(icon->priv->settings,
			       GPM_SETTINGS_MODE_POLICY, 0);
	    mode_choice = 0;
	}
    } else {
	...
	if (mode_choice != 2) {
	    ...
	    ret = gpm_tray_icon_power_policy(2);
	    g_settings_set_int(icon->priv->settings,
			       GPM_SETTINGS_MODE_POLICY, 2);
	    mode_choice = 2;
	}
    }
}
 ...
 **/
static void gpm_tray_icon_init(GpmTrayIcon * icon)
{
    gboolean allowed_in_menu;
    ...
    allowed_in_menu =
	g_settings_get_boolean(icon->priv->settings,
			       GPM_SETTINGS_SHOW_ACTIONS);
    gpm_tray_icon_enable_actions(icon, allowed_in_menu);
    mode_choice =
	g_settings_get_int(icon->priv->settings, GPM_SETTINGS_MODE_POLICY);


    GError *error = NULL;
    icon->priv->power_mode_proxy =
	g_dbus_proxy_new_for_bus_sync(G_BUS_TYPE_SYSTEM,
				      G_DBUS_PROXY_FLAGS_DO_NOT_LOAD_PROPERTIES,
				      NULL, "ukui.power.policy",
				      "/ukui/power/policy",
				      "ukui.power.policy", NULL, &error);
    if (icon->priv->power_mode_proxy == NULL) {
	g_printerr("could not connect to policy system bus: %s", error);
	g_error_free(error);
    } else {
	g_signal_connect(icon->priv->power_mode_proxy,
			 "g-signal",
			 G_CALLBACK(gpm_tray_icon_on_power_policy_signal),
			 icon);
    }
    gboolean auto_enable =
	g_settings_get_boolean(icon->priv->settings,
			       GPM_SETTINGS_AUTO_POLICY);
    gpm_tray_icon_policy_enable_auto(icon, auto_enable);
    gpm_tray_icon_power_mode_restore(icon, mode_choice);

    icon->priv->session = gpm_session_new();
    g_signal_connect(icon->priv->session, "idle-changed",
		     G_CALLBACK(gpm_tray_icon_idle_cb), icon);
}
...
static void gpm_tray_icon_finalize(GObject * object)
{
    ...
    g_object_unref(tray_icon->priv->session);
    g_object_unref(tray_icon->priv->power_mode_proxy);
