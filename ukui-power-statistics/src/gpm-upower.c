 ...
 **/
static const gchar *gpm_upower_get_device_icon_index(UpDevice * device)
{
    gdouble percentage;
...
gchar *gpm_upower_get_device_icon(UpDevice * device)
{
	...
	if (!is_present) {
	    ...
	    filename =
		g_strdup_printf("gpm-%s-%s-charging", prefix, index_str);
...
gchar *gpm_upower_get_device_summary(UpDevice * device)
{
	...
	if (state == UP_DEVICE_STATE_CHARGING
	    ...
	    return g_strdup_printf(_("%s charging (%.1f%%)"), kind_desc,
				   percentage);
	}
	return g_strdup_printf("%s (%.1f%%)", kind_desc, percentage);
    }
    ...
    time_to_full_round =
	egg_precision_round_down(time_to_full, GPM_UP_TIME_PRECISION);
    time_to_empty_round =
	egg_precision_round_down(time_to_empty, GPM_UP_TIME_PRECISION);
	...
	if (kind == UP_DEVICE_KIND_BATTERY
	    ...
	    description =
		g_strdup_printf(_("%s is fully charged"), kind_desc);
	}
	...
	if (time_to_full_round > GPM_UP_TEXT_MIN_TIME &&
	    ...
	    description =
		g_strdup_printf(_("%s %s until charged (%.1f%%)"),
				kind_desc, time_to_full_str, percentage);
	    g_free(time_to_full_str);
	} else {
	...
	description =
	    g_strdup_printf(_("%s waiting to discharge (%.1f%%)"),
			    kind_desc, percentage);
	...
	/* TRANSLATORS: this is only shown for laptops with multiple batteries */
	description =
	    g_strdup_printf(_("%s waiting to charge (%.1f%%)"), kind_desc,
			    percentage);
	...
	description =
	    g_strdup_printf("%s (%.1f%%)", kind_desc, percentage);
    }
...
gchar *gpm_upower_get_device_description(UpDevice * device)
{
    ...
    g_object_get(device,
		 ...
		 "vendor", &vendor,
		 "serial", &serial, "model", &model, NULL);
	...
	if (energy_full > 0 && energy_full_design != energy_full) {
 ...
 **/
const gchar *gpm_device_technology_to_localised_string(UpDeviceTechnology
						       technology_enum)
{
