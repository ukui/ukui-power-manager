...
G_DEFINE_TYPE(MsdMediaKeysWindow, msd_media_keys_window,
	      MSD_TYPE_OSD_WINDOW)
static void msd_osd_window_draw_rounded_rectangle(cairo_t * cr,
						  gdouble aspect,
						  gdouble x, gdouble y,
						  ...
						  gdouble height)
{
    ...
    cairo_arc(cr,
	      x + width - radius,
	      y + radius,
	      radius, -90.0f * G_PI / 180.0f, 0.0f * G_PI / 180.0f);
    cairo_line_to(cr, x + width, y + height - radius);
    cairo_arc(cr,
	      x + width - radius,
	      y + height - radius,
	      radius, 0.0f * G_PI / 180.0f, 90.0f * G_PI / 180.0f);
    cairo_line_to(cr, x + radius, y + height);
    cairo_arc(cr,
	      x + radius,
	      y + height - radius,
	      radius, 90.0f * G_PI / 180.0f, 180.0f * G_PI / 180.0f);
    cairo_line_to(cr, x, y + radius);
    cairo_arc(cr,
	      x + radius,
	      y + radius,
	      radius, 180.0f * G_PI / 180.0f, 270.0f * G_PI / 180.0f);
    cairo_close_path(cr);
}

static void
volume_controls_set_visible(MsdMediaKeysWindow * window, gboolean visible)
{
...
static void
window_set_icon_name(MsdMediaKeysWindow * window, const char *name)
{
...
static GdkPixbuf *load_pixbuf(MsdMediaKeysWindow * window,
			      const char *name, int icon_size)
{
    GtkIconTheme *theme;
    GdkPixbuf *pixbuf;
...
draw_eject(cairo_t * cr,
	   double _x0, double _y0, double width, double height)
{
    int box_height;
    int tri_height;
    int separation;
...
draw_waves(cairo_t * cr,
	   double cx, double cy, double max_radius, int volume_level)
{
    const int n_waves = 3;
    int last_wave;
    int i;
		  ...
		  double _x0, double _y0, double width, double height)
{
    ...
    msd_osd_window_draw_rounded_rectangle(cr, 1.0, 29, 32, 3, bar_width,
					  200);
    cairo_fill(cr);
    cairo_restore(cr);
    ...
    cairo_save(cr);
    cairo_set_source_rgba(cr, 1, 1, 1, 0.8);
    msd_osd_window_draw_rounded_rectangle(cr, 1.0, 29, prog, 3, bar_width,
		   ...
		   double _x0, double _y0, double width, double height)
{
    ...
    gtk_render_background(context, cr, _x0 + 0.5, _y0 + 0.5, x1,
			  height - 1);
...
static void draw_action_volume(MsdMediaKeysWindow * window, cairo_t * cr)
{
    ...
    icon_box_y0 =
	(window_height - icon_box_height - volume_box_height) / 2;
	...
	g_message("speaker box: w=%f h=%f cx=%f cy=%f",
		  speaker_width, speaker_height, speaker_cx, speaker_cy);
#endif

	/* draw speaker symbol */
	draw_speaker(cr, speaker_cx, speaker_cy, speaker_width,
		     speaker_height);
    ...
    draw_volume_boxes(window,
		      ...
		      volume_box_x0,
		      volume_box_y0, volume_box_width, volume_box_height);
}
	      ...
	      double _x0, double _y0, double width, double height)
{
    ...
    if (pixbuf == NULL) {
	char *name;
	if (gtk_widget_get_direction(GTK_WIDGET(window)) ==
	    GTK_TEXT_DIR_RTL)
...
static void draw_action_custom(MsdMediaKeysWindow * window, cairo_t * cr)
{
	...
	draw_volume_boxes(window,
			  ...
			  bright_box_x0,
			  bright_box_y0,
			  bright_box_width, bright_box_height);
    }
}
...
static void msd_media_keys_window_init(MsdMediaKeysWindow * window)
{
	...
	window->priv->progress =
	    GTK_WIDGET(gtk_builder_get_object
		       (builder, "acme_volume_progressbar"));
