...
static gboolean fade_timeout(MsdOsdWindow * window)
{
	...
	gdk_window_invalidate_rect(gtk_widget_get_window(win), &rect,
				   FALSE);
    }
...
static gboolean hide_timeout(MsdOsdWindow * window)
{
...
static void remove_hide_timeout(MsdOsdWindow * window)
{
...
static void add_hide_timeout(MsdOsdWindow * window)
{
    int timeout;
...
static gboolean msd_osd_window_draw(GtkWidget * widget, cairo_t * cr)
{
    MsdOsdWindow *window;
    GtkWidget *child;
...
static void msd_osd_window_real_show(GtkWidget * widget)
{
    MsdOsdWindow *window;
...
static void msd_osd_window_real_hide(GtkWidget * widget)
{
    MsdOsdWindow *window;
...
static void msd_osd_window_real_realize(GtkWidget * widget)
{
    GdkScreen *screen;
    GdkVisual *visual;
...
static void msd_osd_window_style_updated(GtkWidget * widget)
{
    GtkStyleContext *context;
    GtkBorder padding;
    ...
    gtk_container_set_border_width(GTK_CONTAINER(widget),
				   12 + MAX(padding.left, padding.top));
}
				   ...
				   gint * minimum, gint * natural)
{
    ...
    GTK_WIDGET_CLASS(msd_osd_window_parent_class)->
...
static GObject *msd_osd_window_constructor(GType type,
					   ...
					   construct_params)
{
    ...
    object =
	G_OBJECT_CLASS(msd_osd_window_parent_class)->constructor(type,
    ...
    g_object_set(object,
		 ...
		 "skip-pager-hint", TRUE, "focus-on-map", FALSE, NULL);
...
static void msd_osd_window_class_init(MsdOsdWindowClass * klass)
{
	...
	msd_osd_window_get_preferred_height;
...
static void msd_osd_window_init(MsdOsdWindow * window)
{
    GdkScreen *screen;
	...
	gdk_window_get_geometry(gdk_screen_get_root_window(screen), NULL,
				NULL, &sc_width, &sc_height);
