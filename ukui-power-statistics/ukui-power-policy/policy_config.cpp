...
int set_radeon_drm_mode(int status, int mode)
{
    ...
    char *power_dpm_force_performance_level =
	...
	if (mode == 1) {
		...
		if ((fp =
		     fopen(power_dpm_force_performance_level,
			   "w+")) != NULL) {
		    printf
			("open power_dpm_force_performance_level ok!%s\n",
			 level_array[AUTO]);
...
int set_audio_mode(int status, int mode)
{
    ...
    char *audio =
	"/sys/devices/platform/FTHD0001:00/hdaudioC1D0/power/control";
...
//        sprintf(scaling_governor,"%s","ondemand");
	...
	sprintf(path, "%s/%s/%s", scaling_dir, filename->d_name,
		"scaling_governor");
...
//    }
	...
	sprintf(path, "%s/%s/%s", sata_dir, filename->d_name,
